...
7. **System Health Evolution (P1+P2+P3)** — Ausbau `/app/system-health` von Reachability-Dashboard zu vollständigem Ops-Dashboard. P1: Sync-Pipelines + Disk/SSL/PM2 + Deploy-Info sichtbar. P2: 24h-Uptime-Balken (persisted `health_check_log`), Business-Impact-Checks (Webhook-Freshness, Checkout-E2E), Public-Status-Page `/status`. P3: Severity-Routing (Sentry + Resend + optional Slack), Runbooks pro Service. Freigabe: wartet. Plan mit 51 Tasks + Claude-Model-Empfehlung pro Task: [`docs/optimizing/SYSTEM_HEALTH_EVOLUTION_PLAN.md`](optimizing/SYSTEM_HEALTH_EVOLUTION_PLAN.md).
