...
8. **System Health Evolution P2** — Historie + Business-Checks + Public Status Page. `health_check_log` mit 24h-Uptime-Balken + 7d-Prozent. Business-Flows (stripe_webhook_freshness, paypal_webhook_freshness, last_order, active_auctions, checkout_e2e — alle mit Platform-Mode-Awareness + `insufficient_signal`). Public Status Page `vod-auctions.com/status` (auth-frei, Redis-cached 60s, nur Kategorie-Aggregates). Plan §P2. Aufwand ~3-5 Tage.
9. **System Health Evolution P3** — Severity-Routing + Runbooks. Resend-Alerts (immediate für error/critical, Digest 08:00 für warning) + Sentry-Integration + optional Slack. Flapping-Guard (3 consecutive samples) + Cooldown via Upstash. Runbooks P-1 bis P-4 impact-sortiert. Plan §P3. Aufwand ~2-3 Tage.
