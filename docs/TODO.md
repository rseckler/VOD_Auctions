...
0. **[ ] Tier-2 Replica Re-Sync `vod_auctions_replica` (2026-05-08).** Replica out-of-sync seit 2026-05-07, weil `vod_auctions_replication_slot` von Postgres als Schutz gegen Disk-Overflow zwangs-invalidiert wurde (`max_slot_wal_keep_size`, 54 GB WAL akkumuliert). Slot heute morgen gedropped, Subscription disabled, `blackfire_sub` unbetroffen. **Zwischenzeit-Hotfix:** `backup_supabase.sh` Lag-Guard erkennt jetzt `subenabled=false`/`latest_end_time IS NULL` als stale (Commit `3ab0086`) → Backup-Pipeline läuft via Supabase-Direct, Egress kurzzeitig ~39 GB/Mo statt ~1.5. **Nächste Aktion:** Re-Sync per Runbook [`docs/runbooks/RESYNC_VOD_AUCTIONS_REPLICA.md`](runbooks/RESYNC_VOD_AUCTIONS_REPLICA.md) — DROP SUBSCRIPTION + TRUNCATE replizierte Tabellen + CREATE SUBSCRIPTION mit `copy_data=true, create_slot=true`. ~1-2h, ~1.6 GB Egress. Vor Re-Sync: Schema-Drift-Check (`apply_migration`-History seit gestern Abend). Linear: [RSE-323](https://linear.app/rseckler/issue/RSE-323/tier-2-replica-re-sync-vod-auctions-replica). Postmortem: [`docs/sessions/2026-05-08_replica_slot_recovery.md`](sessions/2026-05-08_replica_slot_recovery.md). Memory: `feedback_replica_lag_guard_disabled_subscription.md`.
