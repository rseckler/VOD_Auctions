...
**Letzte Aktualisierung:** 2026-04-26 (rc51.9.1 + rc51.9.2 + rc51.9.3 — **Discogs-Preview-Modal mit Per-Field-Review + JSONB-Binding-Fix + Storefront on-demand-Revalidate.** rc51.9.2 ersetzt silent-overwrite-Refetch durch Review-Modal mit Checkboxen, locked-Felder default OFF. rc51.9.1 fixed JS-Array-zu-JSONB-Serialisierungs-Bug (`format_descriptors` Save crashed mit „An unknown error occurred."). rc51.9.3 schließt 60s-ISR-Cache-Lücke zwischen Edit und Storefront via `revalidateReleaseCatalogPage()`. Frank-Verwirrung dokumentiert: `legacy_condition` ist tape-mag-eigen, hat kein Discogs-Pendant, wird nie überschrieben.)
...
1. **[x] Discogs-Preview-Modal + JSONB-Fix + Storefront-Revalidate (rc51.9.1 + rc51.9.2 + rc51.9.3, 2026-04-26).** **(rc51.9.2)** Neue Route `POST /admin/media/:id/discogs-preview` + `DiscogsReviewModal.tsx` — Discogs-ID-Change triggert Diff-Modal mit Per-Field-Apply-Checkboxen, locked-Felder default OFF mit 🔒-Badge. „Save Linking"-Label flippt zu „Fetch & Review", „Refetch from Discogs" routet ebenfalls durch das Preview. Discogs-Country-Name → ISO-2 via neuem `findCountryByName()`. Out-of-scope: Artist/Label/Cover-Image (dedizierte Picker). **(rc51.9.1)** kritischer Hotfix für Stammdaten-Save: `format_descriptors`-JS-Array wurde direkt an `knex.update()` gegeben, node-postgres serialisierte als PG-`text[]`-Literal, PG kennt keinen `text[] → jsonb`-Cast → Save crashte mit „An unknown error occurred." Fix: `JSON.stringify` vor `trx.update()`. **(rc51.9.3)** Storefront-Catalog-Detail (`revalidate: 60` ISR) bekam stille Stale-Page bis 60s nach Edit. Neuer Helper `lib/storefront-revalidate.ts::revalidateReleaseCatalogPage(id)` ruft den existierenden `/api/revalidate`-Hook fire-and-forget am Ende von `POST /admin/media/:id`. ISR-60s-Safety-Net bleibt. **Frank-Verwirrung dokumentiert in CLAUDE.md:** `legacy_condition` ist tape-mag-MySQL-eigen, hat kein Discogs-Pendant, wird vom Apply nicht berührt. Memory: `project_discogs_review_modal.md` + `project_storefront_revalidate.md` + `feedback_knex_jsonb_array.md`.
