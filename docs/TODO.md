...
**Ziel:** Bestandskunden (CRM-Master 14.450, Tape-mag-Brevo-Liste ~3.580) und neue User können sich registrieren / als Newsletter-Subscriber eintragen, sodass Frank Fortschritts-Updates zu vod-auctions verschicken kann.
**Status:** Backend produktionsreif (Pre-Launch-Invite-System + CRM-Hybrid + Brevo). Storefront-Gateway, DSGVO-Texte und Send-Tooling fehlen.
**Blocker:** Keiner. AGB-Anwalt ist Launch-Blocker für Auctions, NICHT für Newsletter-Versand.
**Nächste Aktion:** Phase A — Public-Sign-up-Form + DSGVO-Texte.

#### Phase A — Storefront-Gateway + DSGVO (vor erstem Versand, ~2h)

- [ ] Public `/newsletter`-Sign-up-Page bauen (`storefront/src/app/newsletter/page.tsx`) — Form (Email + Name optional + Source-Hint) → POST `/store/newsletter` (existiert bereits, Double-Opt-In via HMAC-Token)
- [ ] DSGVO-Einwilligungs-Checkbox + Privacy-Link auf `/newsletter` UND `/apply` ergänzen (kein Default-Tick, Klartext-Einwilligung pro Channel)
- [ ] `/datenschutz`-Page um Newsletter-Section erweitern (Brevo als Auftragsverarbeiter, Retention, Unsubscribe-Rechte, Daten-Export-Hinweis)
- [ ] Unsubscribe-Link in 4 Block-Templates verifizieren (`block-teaser.ts`, `block-tomorrow.ts`, `block-live.ts`, `block-ending.ts`) — Brevo-Token `{unsubscribe_url}` oder Custom-Link?
- [ ] Confirmation-Mail nach Public-Sign-up (Resend-Template `newsletter-confirm.ts` falls noch nicht existent, sonst Wording-Review)
- [ ] Rate-Limit auf POST `/store/newsletter` + `POST /store/waitlist` (verzahnt mit Workstream §4 Redis/Rate-Limiting)

#### Phase B — Bestandskunden-Onboarding (~3h, nach Phase A)

- [ ] Frank reviewt 3 Smart-Lists auf `/app/crm`: 📨 Newsletter Subscribers, 🔕 Unsubscribed, 🌱 Newsletter-Only Leads — verifiziert dass Backfill (3.634 Prefs + 1.954 Auto-Master aus rc53.4) plausibel ist
- [ ] Cross-Check Tape-mag Brevo-Liste 5 (~3.580) gegen `crm_master_email` — Coverage messen, Gaps klären (rc53.4-Backfill sollte das gemacht haben, aber nochmal prüfen)
- [ ] Bulk-Action im CRM-Drawer: "Invite-Token generieren + Email senden" für N selektierte Master-Contacts (nutzt `POST /admin/invite-tokens` per-Row + Resend-Template `invite-welcome.ts`) — neuer Bulk-Endpoint oder Erweiterung der existierenden Smart-List-Bulk-Action
- [ ] Test-Welle: 10-20 Vertraute (Frank + Stamm-Sammler) bekommen Invite-Token, melden sich an, geben Feedback zur Storefront-UX
- [ ] Decision: Bekommt jeder Tape-mag-Subscriber automatisch einen Invite-Token, oder nur auf Anfrage / per Smart-List-Selektion? (Datenschutz-Argument: Tape-mag-Newsletter-Einwilligung deckt nicht automatisch Auction-Plattform-Account)
