...
0. **[in progress] FB-Archive-Pipeline (rc53.11, 2026-05-07).** Frank's vollständiger Facebook-Datenexport (5.819 Posts, 6.369 Media-Files, 11.926 Followers) als Foundation für die spätere Community-Migration. Annex zum Community-Konzept beantwortet die 6 Decisions abgezeichnet. **Live durch:** P1 rsync auf VPS (3.8 GB JSON+HTML), P2 Image-Preprocess (7.310 Bilder zu R2 unter `tape-mag/community-fb/`, 72 % Compression). **Aktuell laufend:** P3 Tier-1-Match gegen VPS-Replica (NICHT Supabase) — `vod_auctions_replica` mit 64k Artists + 52k Releases, Inverted-Index + Pre-Compile, ETA ~11 Min. Erwartete Tier-1-Quote 6,5 % (~290 sicher autorenamebare Posts), Rest geht in Tier 2. **Bereit (ungetestet):** P4 AI-Vision (Haiku 4.5 + tool-use, ~$17 für 3.500 Posts). **Operations-Tracker-Foundation** (universell, `background_job`-Tabelle + `lib/job_tracker.py` + `mark_stale_jobs.py`-Cron) macht alle zukünftigen Long-Running-Jobs sichtbar in `/app/fb-archive` mit Phase-Cards + DB-Health-Banner (color-coded green/yellow/red, Live-TX/Disk-Reads/Cache, persistente Snapshots in /tmp für Delta-Rates) + Slow-Queries-Liste. **Open:** P3-Result-Verify (~10 Min) → P4-Smoke-Test mit `--limit-posts 5 --dry-run` → P4 Live-Smoke (`--limit-posts 10`, ~$0.05) → wenn sauber Full-Run nohup → P5 Manual-Review-CSV-Export. P6 final-DB-Import wartet auf Community-MVP M3+M4+M7 (Phase 1, ~8-12 Wochen). **Doku:** [Annex](Community/Community%20Concept%20—%20Facebook%20Migration%20Annex.md), [Session-Log](sessions/2026-05-07_fb_archive_pipeline.md), [CHANGELOG rc53.11](architecture/CHANGELOG.md). **Memory:** `feedback_hostinger_vps_ipv6_default.md` (IP-Filter-Fall: Hostinger routet outbound default IPv6, IPv4-only Filter blockt alles).
