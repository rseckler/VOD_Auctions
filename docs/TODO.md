...
0. **[ ] Registrierung-Opening + Fortschritts-Newsletter (2026-05-08).** Bestandskunden (Frank/VOD/Tape-mag) und neue User registrieren, damit wir ihnen Newsletter über den Build-Fortschritt von vod-auctions schicken können. **Audit heute (2026-05-08):** Backend ist fertig — Pre-Launch-Invite-System (`/apply` + `/invite/[token]` + `/admin/waitlist` + `POST /admin/invite-tokens` für CRM-Bulk-Generation), CRM↔Newsletter-Hybrid (rc53.4: Brevo-Webhook-Mirror, Drawer-Toggle, 3 System-Smart-Lists), Brevo-Setup (List 4 = VOD, List 5 = Tape-mag/3.580 Subscriber), 4 Block-Templates. **Fehlt:** Public-`/newsletter`-Sign-up-Form (nur Confirm-Page existiert), DSGVO-Checkbox-Texte auf `/apply`+`/newsletter`, Datenschutz-Newsletter-Section, Unsubscribe-Link-Verify in Templates, Fortschritts-Newsletter-Template, Segment-Targeting-UI im Admin-Send. **Kein Launch-Blocker** (AGB-Anwalt nur für Auctions). Volle Aufgabenliste: Workstream §14. **Nächste Aktion:** Phase A starten — Public-Sign-up-Form + DSGVO-Texte (~2h Code).
