...
**Nächste Aktion (Robin):** rc35 committen + deployen, dann auf 3 Macs ausrollen (`bash frank-macbook-setup/install.sh`). Auf Franks Macs wird bei Step 3 altes QZ Tray entfernt (sudo-Prompt fürs `/Applications/QZ Tray.app`-Löschen), dann Bridge installiert. Health-Check: `curl -s http://127.0.0.1:17891/health`.

**Nächste Aktion (Frank):** Nach Rollout — zweiter Test-Durchlauf unter realen Bedingungen: Asmus Tietchens aufrufen, verifizieren, Label drucken (Silent Print via Bridge), Direct Price ändern + Save, Label nochmal drucken. Admin-Header oben rechts muss Badge „Silent Print" zeigen (nicht „Browser Print"). Mittelfristig: Discogs-Mapping Manual Review (`docs/audit_discogs_flagged_2026-04-21.csv`, 431 Items).
