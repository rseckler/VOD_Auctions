...
10. **System Health Observability P4** — Log-Access + Admin-Actions + Alert-History. P4-A (rc44): Log-Drawer mit 4 Tabs (PM2-Logs/File-Logs/DB-Logs/Sentry) + Regex-Scrubbing + SSE-Streaming. P4-B (rc45): Action-Buttons pro ServiceCard (Force-Refresh/Manual-Sync/Acknowledge/Silence/PM2-Restart) mit Audit-Log + Confirmation-Dialog + Rate-Limiting. P4-C (rc46): Alert-History-Panel + Auto-Resolve + Audit-Digest. Freigabe: wartet. Plan: [`docs/optimizing/SYSTEM_HEALTH_OBSERVABILITY_PLAN.md`](optimizing/SYSTEM_HEALTH_OBSERVABILITY_PLAN.md). Aufwand ~5-8 Tage.
