...
**Wichtig:**
- Der `CODE` in `--printer-for CODE=IP` muss **exakt** dem `warehouse_location.code` aus der DB entsprechen (uppercase, ohne Sonderzeichen). Tippfehler routen falsch — und der Lagerort-Default in der Inventur-Edit-Form fällt stumm auf das DB-Default-Lager zurück (rc52.x-Lesson: DB-Code war `EGSTR57/2`, Bridge war `EUGENSTRASSE` → Form defaultete auf ALPENSTRASSE statt EUGENSTRASSE). Seit rc52.3 zeigt die Inventur-Session-Page einen sichtbaren Warning-Banner wenn die Bridge einen Code liefert, der in `warehouse_location` nicht existiert. Workflow für neue Standorte: **erst** den Eintrag in `/app/erp/locations` mit dem gewünschten Code anlegen, **dann** Bridge mit demselben Code installieren.
