...
| `GET /admin/discogs-import/session/:id/status` | JSON | Session-State + letzte 100 Events (für Polling). `since_id` param für incremental fetch |
| `POST /admin/discogs-import/session/:id/cancel` | JSON | Setzt `cancel_requested`, laufende Ops brechen kontrolliert ab |
...
| Datei | Zweck |
|-------|-------|
| `backend/src/lib/discogs-import.ts` | `SSEStream`-Klasse (für analyze/commit), **`emitDbEvent()` helper (für detached fetch loop)**, session helpers (`getSession`, `updateSession`), `expandRow`/`compactRow`, `isCancelRequested`, `awaitPauseClearOrCancel` (nimmt `null` stream für background loops), `pushLastError`, `clearControlFlags` |
| `backend/src/admin/components/discogs-import.tsx` | `useSSEPostReader` hook (analyze/commit), `useSessionPolling` hook (mit `initialLastEventId` param), `ImportPhaseStepper`, `ImportPhaseProgressBar`, `ImportLiveLog`, `SessionResumeBanner`, localStorage helpers |

### Admin UI

**Route-Struktur (seit rc17):**
```
/app/discogs-import                  → Import Wizard (Upload + Analysis Tabs)
/app/discogs-import/history          → Collections-Liste (standalone)
/app/discogs-import/history/:runId   → Run-Detail (standalone)
```

**Wizard `/app/discogs-import`** — nur noch 2 Tabs:
1. **Upload** — Dropzone + Step-2 Fetch Panel + Analyze Integration
2. **Analysis** — Match Review + Import Settings + Live Progress

Oben rechts: "View Collections History →" Button zum Jump in die Liste.
...
Unter laufenden Operationen: **Live-Log-Panel** mit den letzten ~200 Events (aus `import_event` via Polling), Auto-Scroll, Filter (all/progress/errors).

Bei aktiven Operationen: **Cancel** / **Pause** / **Resume** Buttons.

**Collections-Liste `/app/discogs-import/history`:**
- Stats-Header: Total Imports / Total Releases / Inserted / Linked / Updated / Last Import
- Client-side Search über Collection-Name, Source-File, Run-ID
- Tabelle mit pro-Run Stats + CSV-Download-Link pro Zeile

**Run-Detail `/app/discogs-import/history/:runId`:**
- PageHeader mit Back-Link "← Back to Collections", Copy Run ID, Export CSV
- Subtitle inkludiert `source · date · status · inventory: N (yes|no)`
- 8-Karten StatsGrid (Total / Inserted / Linked / Updated / Skipped / Visible now / Purchasable now / Unavailable)
- Import-Settings-Card (Media Condition, Sleeve Condition, Markup, Inventory, Selected IDs count)
- Filter-Bar: Search (Artist/Title/Discogs-ID/Release-ID), Action-Dropdown, Visible-Only-Checkbox
- Release-Tabelle mit Cover (klickbar → `/app/media/:id`), Artist/Title (klickbar), Meta, Action-Badge, Price, Stock-Spalte, Visibility-Dot, Links (🌐 Storefront, D Discogs)
- "Load more" Button für >200 Releases
...
- Beim Page-Load (`loadResumable`): `GET /admin/discogs-import/history` → active_sessions → `GET /admin/discogs-import/session/:id/status?limit=200` → rekonstruiert UI-State (events, progress, collection name, current phase)
- Wenn Session in ACTIVE-State (`fetching`/`analyzing`/`importing`): **Auto-Attach** via Polling, KEIN Banner. Das gesamte Stepper/Progress/Live-Log-UI baut sich aus DB wieder auf als wäre der User nie weg gewesen.
...
export async function POST(req, res) {
  // 1. Validate
  const session = await getSession(pg, session_id)
  if (!session) return res.status(404)...

  // 2. Idempotency check
  if (session.status === "fetching" && lastEventAgeSec < 60) {
    return res.json({ ok: true, already_running: true })
  }

  // 3. Mark session as fetching, emit 'start' event
  await updateSession(pg, session_id, { status: "fetching" })
  await emitDbEvent(pg, session_id, "fetch", "start", { session_id })

  // 4. Return 200 immediately
  res.json({ ok: true, session_id, started: true })

  // 5. Spawn detached loop — runs independently of the HTTP request
  void runFetchLoop(pg, session_id, session, token).catch(async (err) => {
    await updateSession(pg, session_id, {
      status: "error",
...
Der Loop nutzt `emitDbEvent(pg, sessionId, phase, type, payload)` statt `stream.emit()` — schreibt direkt in `import_event` und bumped `last_event_at`. Kein `res`-Reference irgendwo im Loop.

**3 Robustness-Layer:**

1. **Loop unabhängig von HTTP-Request-Lifetime**
   → `res.write()` ist nicht im Hot-Path. Browser kann weg navigieren, Tab schließen, Verbindung cappen — Loop läuft weiter bis Completion/Error/Cancel.

2. **Idempotency-Check verhindert Double-Spawn**
   → Zweiter POST während aktiver Fetch returnt `{ already_running: true }`. Kein paralleler Loop.

3. **Stale-Auto-Restart**
   → Auf Mount prüft die UI `last_event_at`. Wenn >60s alt (pm2 restart, OOM, Crash), re-POSTs zu `/fetch`. Backend erkennt stale-Loop via denselben 60s-Check und startet frisch.

**UI-Konsequenz:**
- `handleFetch()` in page.tsx POSTs, liest 200-JSON-Response (nicht SSE), enabled Polling
- `useSessionPolling` (2s Intervall) ist die einzige Update-Quelle für Fetch-Progress
...
Sessions die in non-terminal Status (`uploaded`/`fetching`/`analyzing`/`importing`) hängen bleiben (z.B. nach pm2 Restart mid-SSE) werden automatisch aus der Resume-Detection excludiert wenn sie älter als 6h sind:

```sql
-- /admin/discogs-import/history active_sessions query
...
Zusätzlich wurde ein neuer terminal state `abandoned` eingeführt (kein Schema-Change — `status` ist `TEXT` ohne Constraint). Operator kann tote Sessions manuell darauf setzen wenn sie sofort aus der UI verschwinden sollen statt auf die 6h-Schwelle zu warten.

### 6. Cancel/Pause via DB-Flags
...
### 7. Commit Hardening v5.1: Per-Batch Transaktionen
...
### 8. Echtes Fuzzy-Matching mit pg_trgm
