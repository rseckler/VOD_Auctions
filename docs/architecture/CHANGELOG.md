...
| **v1.0.0-rc72.2** | 2026-05-17 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Storefront blendet bildlose Artikel aus.** Releases ohne `coverImage` erscheinen in keiner öffentlichen Browse-/Discovery-Oberfläche mehr. Hintergrund (von Frank präzisiert): bildlose Artikel sind nicht im physischen Warenbestand, sondern reiner Referenz-/Schattenkatalog — ~8.700 Releases, davon 0 kaufbar → kein Revenue-Impact. Datenanalyse vorab: `data_source`-Segmentierung zeigt, dass die einzige Bestands-Wahrheit der Inventory Process (`erp_inventory_item`) ist, nicht `coverImage`. **4 Backend-API-Routes:** `catalog/route.ts` Meili-Filter `has_cover=true` (Postgres-Fallback filterte bereits via `whereNotNull("Release.coverImage")`); `band`/`label`/`press`/`[slug]` `whereNotNull("Release.coverImage")` auf Release-/Literatur-/Publikations-Listen **und** Kopf-Zähler. Bereits vorher korrekt: `catalog/{suggest,facets,[id]}`. Bewusst NICHT gefiltert: Account-Seiten (eigene Käufe), Auction-Blocks (Admin-kuratiert), Community (nutzer-kuratiert). Rein additive Read-Filter, keine DB-Änderung. **Verifiziert:** `/store/catalog` total 44.071 = exakt die bebilderten Releases (vorher ~52.788 im `catalog_visibility='all'`-Modus). Analyse + Umsetzung: [`IMAGE_STOCK_DATA_ANALYSIS_2026-05-17.md`](../optimizing/IMAGE_STOCK_DATA_ANALYSIS_2026-05-17.md). **Commit:** `f3c9b90`. |
| **v1.0.0-rc72.1** | 2026-05-17 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Codex-Review-Fixes rc71.5/rc71.6 + Compilation-Tracklist-Rebuild.** Zwei Teile. **(A) Rebuild:** ~2.200 „Various"-Compilations hatten Legacy-`Track.title` als zusammengeklatschten Müll-String (Künstler+Titel+Dauer in einem Feld, mehrere Mangle-Formate). Lifecycle-Trace vorab bestätigt: kein Cron schreibt `Track` → Replace bleibt stehen. Skript `rebuild_compilation_tracklists.py` baut die Track-Rows aus Discogs neu auf (strukturiert: sauberer `title`, separates `artist_name`, `duration`). **2.203 Releases / 38.266 Tracks (37.305 mit Künstler), 0 Fehler;** Meili-Delta reindexed. **(B) Codex-Review** (`codex review --commit` der rc71.5/rc71.6-Commits) — 3 Findings, alle gefixt: **P1** `track_update_release_search_text`-Trigger branched jetzt explizit per `TG_OP` (statt `COALESCE(NEW,OLD)` — robust für DELETE). **P2** der Track-Trigger setzt `search_indexed_at = NULL` mit → jede Track-Änderung (auch roh/scripted) triggert den Meili-Delta-Reindex, kein Caller muss daran denken. **P2** `discogs-import/fetch` cachte die Tracklist nur als `{position,title,duration}` — das Per-Track-`artists`-Array ging verloren, der Bulk-Import konnte den Compilation-Künstler nie komponieren; Cache persistiert jetzt `type_` + `artists` (name/anv/join). **Deploy:** Backend, Trigger-Fix via Supabase MCP, `/health` 200. Typecheck ohne neue Errors. **Commit:** `32effa6`. |
| **v1.0.0-rc72.0** | 2026-05-17 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Discogs-Backfill — Notes (`Release.description`) als 5. Feld.** Frage 2026-05-17: fehlt im Backfill-Tool das Thema „Notes von Discogs"? Ja — der per-Release-`/media/:id`-Flow mappt Discogs-`notes` → `Release.description` (rc71.5: Storefront-„Notes"-Sektion), aber das Bulk-Tool `/app/discogs-backfill` kannte nur Genres/Styles/Credits/Tracklist. **Lifecycle-Trace zuerst** (Konzept-first, [`DISCOGS_BACKFILL_TOOL_KONZEPT.md`](../optimizing/DISCOGS_BACKFILL_TOOL_KONZEPT.md) §Erweiterung): `description` ist das **einzige** Backfill-Feld mit einem permanenten Sync-Writer — `legacy_sync_v2.py` (stündlich) überschreibt es aus MySQL, gated nur durch `locked_fields @> '"description"'`. **Datenlage (5.209 verifiziert+verlinkt):** 197 leere `description`, 2.019 HTML-Scrape-Müll, 2.993 sauber. **Bewusster Scope:** der additive Backfill erreicht nur die **197 echt-leeren** — die 2.019 HTML-Müll-Releases sind nicht leer und bleiben Sache des `/media/:id`-Overwrite-Flows (Trade-off offengelegt). **Umsetzung:** (1) Gap-Scan um `description` erweitert; (1b) **Re-Open** — 63 bereits `applied`-Kandidaten mit leerer `description` werden auf `fetch_pending` zurückgesetzt (Guard `NOT 'description'=ANY(gaps)` gegen Churn); (2) `fetchDiscogsRelease` zieht `notes`; (3) Apply schreibt `description` additiv **+ merged `"description"` in `locked_fields`** (sonst Cron-Overwrite in <1h) **+ `revalidateReleaseCatalogPage`** (schließt auch die latente ISR-Lücke des rc70-Tools für Tracklist/Genres); (4) Admin-UI „Notes"-Spalte. **Geprüft gegen rc69–rc71.6:** kein Konflikt; `discogs_daily_sync.py` + `meilisearch_sync.py` schreiben `description` nicht — nach dem `locked_fields`-Merge läuft kein Sync gegen den Backfill. Keine DB-Migration (`proposed` jsonb / `gaps` text[]). Typecheck: 5 Dateien ohne neue Errors. **Deploy:** Backend + Admin-UI, VPS Vite-Cache-Clear + PM2-Restart. **Commit:** `2144481`. **Offen:** Frank klickt 1× „Re-scan candidates" (~197 Releases, Fetch ~3–4 min). |
| **v1.0.0-rc71.6** | 2026-05-17 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Per-Track-Künstler strukturiert — suchbar + klickbar.** Folge-Bug zu rc71.5: dort war der Compilation-Per-Track-Künstler als `"Artist – Titel"`-String in `Track.title` gebacken — sichtbar, aber nicht suchbar (Suche „David Jackman" fand den Sampler nicht) und nicht klickbar. rc71.6 macht ihn zu einem eigenen Feld. Vollständiges Lifecycle-durchgeplant, Konzept: [`TRACK_ARTIST_STRUKTURIERT_KONZEPT.md`](../optimizing/TRACK_ARTIST_STRUKTURIERT_KONZEPT.md). **Schema:** neue Spalte `Track.artist_name` (Replica zuerst, dann Supabase — schema-weite Publication). **Helper:** `lib/discogs-tracklist.ts::buildTracklist` liefert `artist_name` strukturiert, `title` = reiner Songtitel; genutzt von `discogs-preview`/`discogs-backfill`/`discogs-import/commit`. **Writer:** alle Tracklist-Schreibpfade + Track-CRUD-Routen schreiben `artist_name`; `DiscogsReviewModal` + `TrackManagement` zeigen/editieren ihn. **Klickbar:** `store/catalog/[id]` löst pro Track read-time via `LEFT JOIN LATERAL` auf `Artist.name` einen `artist_slug` auf → Storefront-Tracklist rendert `<Link>` zur Künstler-Seite (wo der Künstler als Entität existiert; sonst Klartext). **Suchbar:** `meilisearch_sync.py` + `meilisearch-push.ts` aggregieren `track_artists` pro Release ins Meili-Doc, `searchableAttributes` erweitert; `search_text`-Trigger + neuer `Track`-Trigger nehmen Track-Künstler auf (Postgres-FTS/Admin). **Remediation:** Migration `unbake_track_artists` — 6.071 gebackene Track-Titel in 420 „Various"-Compilations auseinandergezogen (`split_part` auf erstes `" – "`). **Meili Full-Rebuild** (52.788 Docs) mit dem neuen `track_artists`-Feld, atomic swap. **Bugfix nebenbei:** `wait_for_task` in `meilisearch_sync.py` crashte beim Post-Swap-Schritt — das 5s-404-Toleranzfenster war zu kurz für den swap-indexes-Task nach einem 52k-Doc-Push; 404 wird jetzt über das ganze Timeout-Fenster toleriert (ein synchron gelieferter taskUid ist immer enqueued). **Verifiziert:** „david jackman" findet Flowmotion (VOD-19397) in Meili + `search_text`; Storefront-Tracklist liefert klickbare `artist_slug`. **Deploy:** Backend + Storefront, Replica+Supabase-Migrationen, Meili Settings + Full-Rebuild, Orphan-Staging-Indexe gecleant. **Commits:** `d2df839` (Feature), `4e66cfd` (Arbeitsregel). |
| **v1.0.0-rc71.5** | 2026-05-17 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Discogs-Fetch-Bugfixes — Compilation-Künstler, Notes, Tracklist-Reihenfolge.** Frank-Befund nach rc69-rc71.3. **Bug b [Regression aus rc69.0]:** Fix 2 zog beim Tracklist-Fetch nur position/title/duration — NICHT das Per-Track-`artists`-Array, das Discogs bei Compilations liefert. Ein Refetch einer „Various"-Compilation überschrieb damit `"Algebra Suicide – Somewhat Bleecker Street"` mit nacktem `"Somewhat Bleecker Street"` (die Storefront rendert `Track.title` direkt, Legacy-Daten hatten den Künstler ins Feld gebacken). Neuer Shared-Helper `lib/discogs-tracklist.ts` (`composeTrackArtist` + `buildTracklist`) komponiert den Per-Track-Künstler in den Titel — nur wenn der Track eigene `artists` hat (Single-Artist-Alben bleiben bare). Genutzt von `discogs-preview`, `discogs-backfill`, `discogs-import/commit` (Single Source of Truth, 3 lokale Builder ersetzt). **Bug b2:** Track-Reihenfolge wurde lexikalisch sortiert → `A1, A10, A11, A2` bei Releases mit >9 Tracks/Seite. `store/catalog/[id]` + `admin/media/[id]/tracks` sortieren jetzt natürlich (Seiten-Buchstabe, dann numerischer Teil als Integer, dann volle Position). **Bug a:** Discogs-Notes (`Release.description`) wurden gefetcht (verifiziert: VOD-19396 hat den Notes-Text), aber **nie in der Storefront gezeigt** — `description` lief nur ins SEO-`<meta>`. Neue „Notes"-Sektion auf der Katalog-Detailseite; rendert `release.description`, übersprungen wenn der Rohwert HTML-Tags enthält (Legacy-Scrape-Müll) — selbstheilend, sobald ein Release sauber gefetcht wird. **Remediation:** `scripts/refetch_compilation_tracklists.py` repariert die seit dem rc69-Deploy regressed Compilations — Discogs-Refetch mit Künstler-Komposition, dry-run-default, idempotent. **Deploy:** Backend + Storefront, VPS Vite-Cache-Clear + `.next/cache`-Clear + PM2-Restart. Typecheck: 8 Dateien ohne neue Errors. **Commit:** `0b816ef`. |
| **v1.0.0-rc71.4** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Erweiterung 02 — Post-Detail-Cover + Cache-Clear.** Folge zu rc71.2. **Cover (Robin-Befund):** die Post-Detailseite (`/community/post/[slug]`) zeigte gar kein Cover, weil alle Demo-Editorials `cover_image_url = NULL` haben und das Bild nur `{cover_image_url && …}` gerendert wurde. Jetzt: ist kein echtes Bild da, zeigt ein dekorativer Cover-Band-Platzhalter (`.cm-post-detail-cover-ph`, 200px, `.cm-cover-art`-Gradient + „From the Archive"-Caption) — analog zu den Feed-Karten. **Cache:** Der „Frank Maier"-Rest auf der Post-Detailseite war **kein Datenfehler** (DB-Verifikation: alle Editorials `author = Frank Bull` seit rc71.2) sondern stale ISR-Fetch-Cache (`community-api.ts` `revalidate: 60`). Deploy daher mit `rm -rf .next/cache` vor dem Build → alle Community-Seiten re-fetchen frisch, Frank Bull überall sofort. **Deploy:** Storefront-only, VPS `git pull` + `.next/cache`-Clear + `npm run build` + PM2-Restart. **Commit:** `da70e1f`. |
| **v1.0.0-rc71.3** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Codex-Review-Fixes für rc69/rc70 — 3 Findings.** Codex-Review (`codex review --commit`) der beiden Discogs-Commits dieser Session. **F1 [P1] Tracklist-Wipe (rc69.0, live):** Lieferte Discogs keine verwertbare Tracklist, war `proposed.tracklist=[]` — für ein Release mit lokalen Tracks entstand ein per Default angehakter `[N Tracks] → []`-Diff, dessen Apply den Replace-Pfad (`DELETE FROM "Track"` + 0 Inserts) erreichte → **Tracklist gelöscht**. Fix: `discogs-preview` schlägt eine leere Tracklist gar nicht erst als Diff vor; `media/[id]`-Apply ersetzt Track-Rows nur noch bei `length > 0` (Defense-in-Depth). **F2 [P2] `job_running` hing nach Backend-Restart (rc70.0):** GET meldete `job_running` anhand `fetch_pending > 0` — nach einem Restart/Crash mit offenen Rows war kein Worker aktiv, die UI sperrte aber den Resume-Button und pollte ewig. Fix: `prepareRunning`-Flag in `lib/discogs-backfill.ts` gezogen (GET + prepare im selben Prozess), GET meldet den echten Worker-State + neues `stalled`-Flag; die UI zeigt dann „Resume fetch". **F3 [P2] Markt-only-Apply blockiert (rc69.0):** der Apply-Button im `DiscogsReviewModal` war bei 0 ausgewählten Feldern immer disabled — ein reiner Marktpreis-Refresh war unerreichbar, sobald Metadaten-Diffs existierten. Fix: Button bleibt aktiv wenn Marktpreise vorliegen. **Deploy:** Backend + Admin-UI, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200. Typecheck: 7 geänderte Dateien ohne neue Errors. **Commit:** `b42a444`. |
| **v1.0.0-rc71.2** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Erweiterung 02 — Korrekturrunde nach Robins Review (rc71.0).** **Feed-Architektur:** rc71.0 hatte den Feed auf eine randlose Hairline-Liste umgebaut (= Claude-Design „After"-Artboard); Robin verglich gegen die „Before"-Artboard (Karten) → Entscheidung: **kompakte Karten**. `.cm-post` ist wieder eine Karte (Rahmen/Rundung/`var(--card)`-Hintergrund) mit dem dichten Innenraum aus rc71.0 (14px Padding, 36px Avatar, 14px-Body); `.cm-feed` gap 0→10px; Editorial-Card = Karte mit Gold-Topline + gold-getöntem Rahmen. **Editorial-Hero im Hub wiederhergestellt** (rc71.0 hatte ihn entfernt). **Cover-Platzhalter (Bug a):** das Cover-Thumbnail wird jetzt **immer** gerendert, wenn ein Post ein Release verlinkt oder ein Bild hat — fehlt das echte Bild, zeigt ein dekorativer CSS-Platzhalter (`.cm-cover-art`, Vinyl-Culture-Gradient) die Verknüpfung statt einer Leerstelle; Editorials bekommen immer ein Cover (Feed-Card + Hero). **Release-Detail-Community-Sektion verdichtet (Bug b):** Stats-Strip (Padding 22/28→13/18, Margin 32/24→14), Rating-Overview (gap 32→24, Score-Zahl 44→30px), Histogramm (fixe Zeilenhöhe 16px + `white-space:nowrap` am Label — fixt den Wrap-Bug, der „5"+„★" auf zwei Zeilen umbrach und die Zeilen aufblähte), Section-Titel 26→20px, Rating-Picker-Sterne 24→20px. **Demo-Curator (Bug c):** „Frank Maier" → **„Frank Bull"** — `community_profile`-Profil `cmpro_demo_frank` (display_name + Handle `frankmaier`→`frankbull`) und `scripts/community_seed.py`. **Deploy:** Storefront-only, VPS `git pull` + `npm run build` + PM2-Restart `vodauction-storefront`, Storefront-Build grün. **Commit:** `4cbc39e`. |
| **v1.0.0-rc71.1** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Hotfix: Tier-2-Replikation stand ~7 min — rc70.0-Tabelle fehlte auf der Replica.** Alert per Email: `vod_auctions_sub` enabled, `pg_stat_subscription.latest_end_time` NULL. **Root Cause:** `vod_auctions_pub` ist eine **schema-weite** Publication (`FOR TABLES IN SCHEMA public`) — die rc70.0-Tabelle `discogs_backfill_candidate` wurde dadurch automatisch publiziert, aber nicht zugleich auf der `pg17-replica` angelegt. Beim ersten Write (Frank startete „Build candidate list") streamte der INSERT zur Replica → Apply-Worker `target relation "public.discogs_backfill_candidate" does not exist` → Crash-Loop alle 5 s, gesamte `vod_auctions_sub`-Replikation gestoppt. Backups liefen korrekt über den Supabase-Direct-Fallback (rc53.13 Lag-Guard). **Slot war gesund** (`wal_status: reserved`, kein `wal_removed`) → kein Re-Sync nötig. **Fix:** Tabelle auf der Replica nachgezogen + `ALTER SUBSCRIPTION vod_auctions_sub REFRESH PUBLICATION WITH (copy_data=false)`. Worker erholt, `latest_end_time` füllt, Slot-WAL 171 MB → 16 MB, `pg_subscription_rel` 285 alle `r`, Replica 1.246 Rows ↔ Source caught-up. **Prävention:** CLAUDE.md DB-Gotcha „Logical Replication" ergänzt — jede neue `public`-Tabelle MUSS vor dem ersten Write auch auf der Replica angelegt werden (schema-weite Publication, kein Opt-out); die rc70.0-Notiz „nicht in vod_auctions_pub" war falsch und ist im Konzept richtiggestellt. **Kein Deploy** (reine DB-Ops + Doku). |
| **v1.0.0-rc71.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Erweiterung 02 — visuelle Verdichtung (Density Revision).** Aus [`Erweiterungen.md`](../Community/Erweiterungen.md) §02. Workflow: Density-Konzept ([`ERWEITERUNG_02_KONZEPT.md`](../Community/ERWEITERUNG_02_KONZEPT.md)) → Design-Handoff an Claude Design ([`ERWEITERUNG_02_DESIGN_HANDOFF.md`](../Community/ERWEITERUNG_02_DESIGN_HANDOFF.md)) → der Claude-Design-Output (Bundle `7xW0G8VG…`, 4 Screens Before/After) wurde in den Next.js-Code übersetzt. **Befund (Robin):** Feed zu großzügig — Postboxen/Abstände/Schrift zu groß, ~1,5 Einträge pro Screen. **Feed:** Card-Chrome raus → **Hairline-Liste**: `.cm-post` ist eine dichte Zeile (14px Padding, 1px-Trennlinie, kein Card-Hintergrund/Radius), Body 14px/1.45 mit 4-Zeilen-Clamp + „… more", Avatar im Feed 48→36px, Aktionszeile 32px, `.cm-feed` gap 32→0. **Cover:** 72px Quadrat-Thumbnail rechts an jedem Post mit Release/Bild; die Release-Verknüpfung darunter als kompaktes **Referenz-Pill** (Mini-Cover + Titel + Artist + →) statt großem Inline-Block. **Editorial im Feed:** dichte Zeile, abgesetzt nur durch Gold-Topline + 18px Serif-Titel (`.cm-post.cm-post-editorial`); der separate Hub-Hero entfällt — Editorials laufen inline im Feed. **Mobile:** neue fixe **5-Slot-Bottom-Tab-Bar** (`CommunityBottomNav` — Feed · Explore · Compose⁺ · Members · Inbox, mittiger gold-gehobener Compose-Button), ≤900px sichtbar; der Compose-FAB ist auf Mobile ausgeblendet. **Profil:** kompakter Header (Avatar 120→96px, Name 36→28px, Inline-Stats statt großer Serif-Zahlen), Featured-Releases als 52px-Cover-Reihe statt großer Karten. **Post-Detail bewusst unverändert** — die Leseansicht bleibt großzügig (Kontrast: dicht im Feed, luftig im Lesen). **Keine Backend-/DB-Änderung** — reine Storefront-CSS + Komponenten. **Neue Komponente:** `CommunityBottomNav`. **Deploy:** Storefront-only, VPS `git pull` + `npm run build` + PM2-Restart `vodauction-storefront`, `/health` 200. Storefront-Build grün (alle Community-Routen kompilieren). **Commit:** `71d4dfc`. |
| **v1.0.0-rc70.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Discogs-Metadaten-Backfill — Review-Tool.** Konzept: [`DISCOGS_BACKFILL_TOOL_KONZEPT.md`](../optimizing/DISCOGS_BACKFILL_TOOL_KONZEPT.md), Datengrundlage: [`VERIFIED_RELEASES_DISCOGS_GAP_ANALYSIS_2026-05-16.md`](../optimizing/VERIFIED_RELEASES_DISCOGS_GAP_ANALYSIS_2026-05-16.md). **Hintergrund:** Gap-Analyse der 5.211 verifizierten Releases — 1.246 sind mit Discogs verlinkt, haben aber fehlende Genres/Styles/Credits/Tracklist (`discogs_daily_sync.py` schreibt nur Preise, nie Metadaten). **Tool** (`/app/discogs-backfill`, Sidebar-Shortcut „Discogs Backfill"): zeigt alle Kandidaten in einer Review-Tabelle `current → proposed` je Feld — Frank akzeptiert pro Zeile oder im Bulk, **nichts passiert automatisch**. **Staging-Tabelle** `discogs_backfill_candidate` (Migration via Supabase MCP — transient, bewusst nicht in `vod_auctions_pub`, da aus Discogs rekonstruierbar). **`POST /prepare`:** Scan der Kandidaten + rate-limitierter Discogs-Hintergrund-Fetch (~1,1 s/Call ⇒ ~23 min für 1.246, HTTP-entkoppelt per `void (async…)()`, resume-fähig). **`POST /apply`:** schreibt **rein additiv** — nur Felder die JETZT noch leer sind (Re-Check beim Apply, nicht beim Scan), Tracklist nur bei 0 `Track`-Rows, `search_indexed_at=NULL`-Bump für Meili; jede Release in eigener Transaktion. **`POST /reject`:** Listen-Hygiene. **Scope:** Marktpreise sind NICHT Teil des Tools — die deckt der `discogs_daily_sync.py`-Cron ab. **Neu:** `discogs_backfill_candidate`-Tabelle, `backend/src/lib/discogs-backfill.ts`, 4 Admin-Routen, Admin-Page `discogs-backfill`. **Deploy:** Backend + Admin-UI, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200. Typecheck: alle neuen Dateien ohne Errors. **Commit:** `c15a711`. **Offen:** Frank baut die Kandidatenliste auf (1× „Build candidate list", ~23 min Fetch) und reviewt. |
| **v1.0.0-rc69.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Inventory Discogs Fetch — Marktpreis-Bugfix + Tracklist + Linking direkt im Erfassungs-Tab.** Konzept: [`INVENTORY_DISCOGS_FETCH_KONZEPT.md`](../optimizing/INVENTORY_DISCOGS_FETCH_KONZEPT.md). **Bug (Fix 1):** Frisch über den Katalog verlinkte Discogs-Releases zeigten im Inventory-Process keinen „Markt aktuell / Discogs-Suggestion"-Block. Root Cause: der Katalog-Discogs-Flow läuft seit rc51.9.2 über den Review-Modal, dessen Apply-Pfad die vier `discogs_*_price`-Felder nie schrieb (2026-05-07 „M3" hatte den Marketplace-Fetch entfernt) — Preise kamen nur vom nächtlichen `discogs_daily_sync.py`-Cron, also Tage später. **Fix:** `discogs-preview` fetcht `/marketplace/stats` + `/price_suggestions` wieder, liefert ein nicht-reviewbares `market`-Objekt; der Apply-Pfad (`POST /admin/media/:id`) akzeptiert die vier Preis-Spalten (kein Lock/Audit/Checkbox — Markt-Referenz) + stempelt `discogs_last_synced`. **Tracklist (Fix 2):** der Katalog-Discogs-Fetch zog nie eine Tracklist — `discogs-preview` diffed jetzt `tracklist` (aus `apiData.tracklist`, gegen die `Track`-Tabelle), der Apply-Pfad ersetzt die `Track`-Rows wenn angehakt; `isEqual` rekursiv (Objekt-Arrays). **Credits:** kein Gap — wurden schon immer aus `extraartists` gezogen. **Erweiterung b:** der Erfassungs-Tab (`/app/erp/inventory`) hat jetzt eine **immer sichtbare „DISCOGS"-Sektion** im Release-Panel — ID-Feld auch ohne Link (statt „nichts"), immer editierbar (Relink/Korrektur); Fetch/Refetch öffnet denselben `DiscogsReviewModal` wie der Katalog, danach Auto-Reload des Release-Detail. `release/:id/copies` liefert `discogs_id` raw; Inventory-Shortcut-Handler pausiert bei offenem Modal. **Neu im Modal:** `TracklistCell`, Markt-Info-Zeile, „Refresh market prices" im No-Changes-Fall. **Deploy:** Backend + Admin-UI, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200. Keine DB-Migration (alle Spalten/Tabellen existieren). Typecheck: 6 geänderte Dateien ohne neue Errors. **Commit:** `a17dea7`. |
| **v1.0.0-rc68.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Erweiterung 01 — Post-Bearbeitung + Composer-Hinweis; + Erweiterung-02-Scroll-Fix.** Aus [`Erweiterungen.md`](../Community/Erweiterungen.md). **Post-Bearbeitung:** `posts/[id]` PATCH akzeptiert jetzt `release_id` + `artist_id`/`label_id`/`press_id` — der Artikel-Link eines Posts ist editierbar, nicht nur bei der Erstellung setzbar. Die Compose-Seite hat einen **Edit-Modus** (`?edit=<id>`): lädt den Post, füllt Titel/Body/Tags/Cover/Release vor, speichert per PATCH; `PostEditor` nimmt `initialContent`. **Edit-Link** auf der Post-Detailseite (`EditPostLink`), nur für den Autor sichtbar. **Composer-Hinweis:** Hinweis-Box im Composer — ein Foto, ein Video-Embed oder zumindest ein verlinktes Release macht einen Post deutlich stärker (Instagram-Logik, Erweiterung 01). **Erweiterung 02 (erster konzept-unabhängiger Fix):** `.cm-root { overflow-x: clip }` — die Community-Seiten lassen sich nicht mehr horizontal bewegen; `clip` statt `hidden`, damit die Sticky-Sub-Nav nicht bricht; lange Strings brechen um (`overflow-wrap: anywhere`). **Doku:** [`SOCIAL_UX_REFERENCE.md`](../Community/SOCIAL_UX_REFERENCE.md) — konkrete Social-Plattform-UX-Patterns (Dichte, Typo-Skala, Card-Anatomie, Mobile-Nav) als Input für Robins Visuelle-Optimierungs-Konzept (Erweiterung 02). Browser-Screenshots waren in dieser Umgebung nicht möglich — das Dokument ist die textuelle Entsprechung. **Neue Komponente:** `EditPostLink`. **Deploy:** Backend + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `4cb5a93`. **Offen:** Erweiterung 02 Kern (Density-/Mobile-Umbau) — wartet auf Robins Konzept. |
| **v1.0.0-rc67.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R9 — Featured-Releases, Privacy-Toggles, Entity-Walls.** Letzte Phase des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md) — reiner Code, das Schema kam gebündelt in rc63. **Top-4-Featured-Releases:** Picker in `/community/settings` (bis zu 4 Releases über die Katalog-Suche), gepinnte Anzeige-Leiste im Profil-Header (`featured_releases`). **Privacy-Toggles:** Settings-Toggle „Show my collector tier" — angewendet: blendet das Tier-Badge auf dem öffentlichen Profil aus; `show_acquired_feed` gated zusätzlich den Acquired-Tab des öffentlichen Profils (Toggle bereits seit rc64). **Entity-Walls:** Posts können an Band/Label/Press geankert werden (`community_post.artist_id`/`label_id`/`press_id`, Schema seit rc63) — Posts-Route filtert danach; neue `EntityWall`-Komponente auf den Band-, Label- und Press-Seiten zeigt die geankerten Community-Posts + Write-a-post-Link; der Composer liest `artist_id`/`label_id`/`press_id`-Parameter. **Neue Komponente:** `EntityWall`. **Damit ist der Community-Rebuild-Plan (R0–R9) vollständig.** **Deploy:** Backend + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; verifiziert: Posts-Route nimmt `artist_id`-Filter, Profil-Route liefert `featured` + Privacy-Flags. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `8fef374`. |
| **v1.0.0-rc66.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R8 — Trust-Levels + Moderations-Admin-Politur.** Phase R8 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). **Trust-Level-Auto-Promotion-Job:** neuer Scheduled Job `community-trust-levels` (täglich 03:30 UTC) — berechnet `community_profile.trust_level` (TL0–TL3, Concept §8.2) für **jeden** Member in einem Bulk-Durchlauf aus Account-Alter + Aktivität (Posts + Comments). Bisher wurde `trust_level` nur lazy bei Post-Erstellung aktualisiert — ein lesend/kommentierend aktiver Member wurde nie befördert. **Community-Admin-Politur (`/app/community`):** Dashboard zeigt die Trust-Level-Verteilung (TL0–TL3-Counts); Moderations-Queue — Report-Zeilen verlinken jetzt auf den gemeldeten Post (`target_slug` aus der Reports-Route), Posts-Tab bekommt Multi-Select mit Bulk-Hide / Bulk-Remove. **Rate-Limits** (`dailyPostLimit`, TL0 5/Tag · TL1 20/Tag) sind bereits in der Post-Route aktiv — verifiziert, kein Change nötig. **Bewusst aufgeschoben** (Feature-Slices, für einen betreibbaren Admin nicht nötig): Editorial-Scheduling, Curated-Tags-Tabelle, CRM-Customer-„Community"-Tab, Claude-Haiku-Auto-Mod. **Deploy:** Backend-only (1 neuer Job auto-registriert, 2 Admin-Routen, Admin-UI), VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; Admin-Route 401 unauth (Auth-Gate korrekt). Typecheck Backend ohne neue Errors. **Commit:** `ca19217`. |
| **v1.0.0-rc65.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R7 — Suche + Explore.** Phase R7 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). **Suche:** neue Route `GET /store/community/search` — Postgres-ILIKE über Posts (Titel/Excerpt/Tags), Member (Handle/Name) und Lists (Titel/Beschreibung), demo-gegated. **Bewusst kein Meili-Index:** Community-Volumen (einige tausend Posts) braucht keine separate Meili-Infrastruktur — Postgres-ILIKE ist hier ausreichend und vermeidet Index-Sync-Cron + `search_indexed_at`-Trigger. **Explore-Seite:** `/community/explore` von der Coming-Soon-Halteseite ausgebaut — Such-Box mit live-debounced Ergebnissen (Posts/Members/Lists) und eine Discovery-Ansicht ohne Query (Trending-Tags, Popular-Lists, Members-to-follow). **Neue Komponente:** `ExploreBrowser`. **Deploy:** Backend (1 Store-Route) + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; verifiziert: `search?q=zko` → 5 Posts/2 Member/1 List, `q=tape` → 10/2/2. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `9c07c6f`. |
| **v1.0.0-rc64.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R6 — Acquired-Feed, Bookmarks, Notifications, Email.** Die Retention-Schicht; Phase R6 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). **Schema** (main + Tier-2-Replica): Tabelle `community_saved`; `community_notification.is_emailed`; `community_profile.email_notifications`. Plus: CHECK-Constraint `community_post_kind_check` um `'acquired'` erweitert (memory `feedback_check_constraint_action_drift` — auf Replica + main migriert). **Bookmarks:** Save/Unsave-Route + Saved-Liste, Save-Button auf Post-Detail, `/community/saved`-Ansicht. **Notifications:** Live-Unread-Dot am „Community"-Header-Nav-Eintrag (`CommunityNavDot`). **Acquired-Feed:** Scheduled Job `community-acquired-feed` scannt bezahlte Transaktionen und erzeugt automatisch `kind='acquired'`-Posts für Member, die opt-in sind (`community_profile.show_acquired_feed`) — **bewusst vom Payment-Webhook entkoppelt** (Job scannt statt Geld-Code zu hooken, ein Fehler kann die Bestellabwicklung nicht beeinträchtigen). Acquired-Posts mit „Acquired"-Tag + Profil-Acquired-Tab. **Email:** Job `community-notification-emails` (alle 2 h) — Pro-Empfänger-Digest un-ge-mailter Notifications, Opt-out via `email_notifications`; Job `community-weekly-digest` (sonntags) — Top-Posts der Woche. Beide via Resend (`sendEmailWithLog`), in `/app/emails` registriert. **Settings:** Profil-Route + UI um „Share acquisitions" + „Email notifications"-Toggles erweitert; Profil-API exponiert zusätzlich `featured_releases`/`show_tier`/`show_wantlist` (für R9). **Demo-Seed** um 9 Acquired-Posts erweitert. **Deploy:** Backend (3 neue Jobs auto-registriert, 1 Store-Route) + Storefront, VPS Vite-Cache-Clear + PM2-Restart, Seed neu geladen, `/health` 200; verifiziert: Feed liefert 9 Acquired-Posts, Replica repliziert sie, Subscription caught-up. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `4c2f564`. |
| **v1.0.0-rc63.1** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Hotfix: Member-Profil-Seite crashte (fehlendes Post-`author`-Objekt).** Sentry-Issue [#120272182](https://sentry.io) — `TypeError: undefined is not an object (evaluating 'e.display_name')`, Safari, 1×. **Diagnose:** Der Sentry-`transaction`-Tag zeigte irreführend `/community/explore`; der echte Stacktrace (`metadata.filename` + `tags.url`) wies auf `CommunityUI.tsx::Byline` auf `/community/members/cassetteculturejp`. **Root Cause:** `GET /store/community/profiles/:handle` lieferte die `posts` ohne `author`-Objekt (SELECT enthielt keine Author-Felder). Die Member-Profil-Seite reicht diese Posts in die geteilten Feed-Cards `PostCard`/`EditorialCard` → `Byline`, die `author.display_name` unbedingt lesen → die ganze Profilseite wurde weggerendert (White-Screen). **Fix:** Da jeder Post einer Profilseite von genau diesem Profil stammt, hängt die Route `serializeProfile(profile)` als `author` an jeden Post — dieselbe Shape, die die Post-Create-Route bereits zurückgibt. **Verifiziert:** `/store/community/profiles/cassetteculturejp` liefert beide Posts mit `author.display_name`. **Files:** `backend/src/api/store/community/profiles/[handle]/route.ts`. **Commit:** `d26e3a3`. **Deploy:** Backend-only, VPS Vite-Cache-Clear + PM2-Restart, Backend-Build mit bekannten pre-existing TS-Errors (keine neuen), `/health` 200. **Release-Tag:** [v1.0.0-rc63.1](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc63.1). |
| **v1.0.0-rc63.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R5 — Lists (kuratierte Sammlungen)** + gebündelter Schema-Slice. Phase R5 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). **Schema:** neue Tabellen `community_list` + `community_list_item` (Migration `community_lists_r5`). **Auf der Tier-2-Replica gespiegelt** (`pg17-replica`-Container, `vod_auctions_replica`), zu `vod_auctions_pub` hinzugefügt, `ALTER SUBSCRIPTION … REFRESH PUBLICATION` — Replikation der neuen Tabellen verifiziert (`pg_subscription_rel` state `r`, 6 Lists/32 Items repliziert). **Gebündelter Schema-Slice** (Owner-Wunsch, damit spätere Phasen keine weitere Migration brauchen — additiv, main + Replica): `community_profile` += `featured_releases jsonb`, `show_tier`, `show_acquired_feed`, `show_wantlist` (für die aufgeschobenen Top-4-Featured + Privacy-Toggles); `community_post` += `artist_id`, `label_id`, `press_id` (für die aufgeschobenen Band/Label/Press-Walls). **Backend:** Lists-CRUD + Items-Routen — `lists` (GET Liste / POST anlegen), `lists/[id]` (GET Detail / PATCH / DELETE), `lists/[id]/items` (POST add / DELETE remove); demo-gegated, Public/Private-aware. **Storefront:** `/community/lists`-Directory (aus der Coming-Soon-Halteseite ausgebaut), `/community/lists/new` Anlege-Flow, `/community/lists/[id]` Detail mit Owner-`ListManager` (Release über Picker hinzufügen, entfernen, Liste löschen); Lists-Tab im Member-Profil. **Neue Komponenten:** `ListCard`, `ListManager`. **Demo-Seed:** `community_seed.py` um 6 Demo-Lists erweitert (32 Items). **Deploy:** Backend (3 Lists-Routen) + Storefront, VPS Vite-Cache-Clear + PM2-Restart, Seed neu geladen, `/health` 200; verifiziert: Directory 6 Lists, Detail lädt Items, Replica repliziert. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `39b25a5`. |
| **v1.0.0-rc62.1** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **System-Health-Probe `database_io` — invalide Indizes + Cache-Hit automatisch überwacht.** Folge-Maßnahme zu rc60.2: damit ein kaputter Index (fehlgeschlagener `CREATE INDEX CONCURRENTLY` → `indisvalid=false`-Hülle, vom Planner lautlos ignoriert) und Disk-IO-Druck nicht mehr manuell auffallen müssen. **Kein neuer Cron** — neuer Eintrag in der bestehenden `CHECKS`-Registry (`backend/src/lib/health-checks.ts`), läuft im vorhandenen System-Health-Sampler mit, nutzt Alerting + Admin-UI (`/app/system-health`) unverändert. **Checks:** (a) invalide Indizes via `pg_index.indisvalid` → `error` mit Index-Namen in `metadata`; (b) Cache-Hit-Ratio aus `pg_stat_database` als Disk-IO-Frühwarnsignal → `ok ≥95%`, `warning 90–95%`, `error <90%`. Combined-Status = schlechterer der beiden. `check_class: background`, Kategorie `infrastructure`. **Neuer Runbook** [`docs/runbooks/database-io.md`](../runbooks/database-io.md) — Diagnose + Fix für beide Fälle (Duplikat-Check → `DROP` → `CREATE … CONCURRENTLY` → verifizieren; `pg_stat_statements`-Top-Verbraucher). **Verifiziert nach Deploy:** Probe-Queries liefern `invalid_indexes: 0`, `cache_hit_pct: 98.38%` → Status `ok`. **Files:** `backend/src/lib/health-checks.ts`, `docs/runbooks/database-io.md`. **Commit:** `8c1eba9`. **Deploy:** Backend-only, VPS Vite-Cache-Clear + PM2-Restart, Backend-Build mit bekannten pre-existing TS-Errors (keine neuen), `/health` 200. **Release-Tag:** [v1.0.0-rc62.1](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc62.1). |
| **v1.0.0-rc62.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R4 — Catalog-Anchored Discussion + Reviews.** Phase R4 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). Der katalog-geankerte Community-Block auf der Release-Detailseite (`ReleaseCommunitySection`) ist auf Mockup-Screen 04 umgebaut. **Stats-Strip:** Reviews-Count · Ø-Rating · Discussions-Count. **Rating-Overview + Histogramm:** großer Ø-Score + Sterne + RYM-artiges Verteilungs-Histogramm (5→1, `RatingHistogram`). **Sortierbare Review-Liste** (`ReleaseReviews`-Client-Komponente): Recent / Highest-rated / Verified-owners — `GET /store/community/reviews` nimmt jetzt einen `sort`-Param und liefert zusätzlich das `histogram` (Count je Rating 1–5). **Discussion-Thread** mit Eyebrow + Write-a-post-Link. Verifizierte Reviewer behalten den `✓ Owned`-Pill. **Neue Komponenten:** `RatingHistogram`, `ReleaseReviews`. **Offen (schema-pflichtig):** Band-/Label-/Press-„Wall"-Tabs — `community_post` ankert nur an `release_id`, Entity-Walls bräuchten zusätzliche FK-Spalten; Own/Want-Member-Drawer braucht die Wantlist-Brücke. **Deploy:** Backend (Reviews-Route) + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; verifiziert: `reviews?sort=top` liefert Histogramm + Counts. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `830ba62`. |
| **v1.0.0-rc61.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R3 — Member-Directory, Profil-Tabs, Onboarding.** Phase R3 des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). **Member-Directory (`/community/members`):** neue Route `GET /store/community/members` — Liste mit Tier-Filter, Sortierung (Aktivität/Neueste/Tier), Handle-/Namens-Suche und Tier-Tally für die Toolbar-Pills. Die Directory-Seite von der Coming-Soon-Halteseite auf Mockup-Screen 05 ausgebaut: Tier-Filter-Pills mit Counts, Suche, Sort, Member-Cards-Grid (`MembersBrowser`-Client-Komponente, debounced Refetch). **Profil-Tabs:** `profiles/[handle]` liefert jetzt zusätzlich die letzten Comments + Reviews des Members; die Profil-Seite bekommt getabten Inhalt — **Posts / Comments / Reviews** (`ProfileTabs`), mit Comment-Cards („on {Post}") und Review-Cards (Rating-Stars + Release). Eigenes Profil zeigt „Edit profile". Lists-/Acquired-/Wantlist-Tabs folgen mit ihren Phasen (R5/R6). **Onboarding:** neue Seite `/community/onboarding` — First-Run-Profil-Setup (Foto, Display-Name, Handle, Location, Sammler-seit, Bio); dismissbarer Hub-Nudge (`OnboardingNudge`) für Member mit unkonfiguriertem Profil. **Neue Komponenten:** `MembersBrowser`, `ProfileTabs`, `OnboardingNudge`. **Deploy:** Backend (neue Store-Route) + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; verifiziert: Directory liefert 13 Member + Tier-Counts, Profil-Tabs (`discoveredzkoin1989`) 2 Posts/9 Comments/1 Review. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Offen (spätere Slices):** Top-4-Featured-Releases + Privacy-Toggles (beide schema-pflichtig), Block-Liste. **Commit:** `9d81822`. |
| **v1.0.0-rc60.2** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Disk-IO-Fix: invaliden Unique-Index repariert + Mail-Import-Dashboard gecacht.** Supabase meldete Disk-IO-Budget-Erschöpfung auf `vod-auctions`; Treiber war das CRM-Mail-Import-Subsystem (RSE-322, neu seit der letzten IO-Sweep rc52.6.5). **Root Cause:** `idx_crm_imap_message_msgid_unique` war durch einen fehlgeschlagenen `CREATE INDEX CONCURRENTLY` als `invalid`/0-Byte-Hülle hinterlassen — der Planner ignorierte ihn lautlos, der Dedup-Check des Imports (`SELECT … WHERE message_id_header = ANY(…)` in `import_legacy_mails_v3.py`) lief seitdem als Parallel Seq Scan über 407k `crm_imap_message`-Rows bei **jedem** Batch (#1-IO-Verbraucher der DB: 93 Mio. Disk-Blocks über 8.372 Aufrufe, nur 56 % Cache-Hit). Der Script-Kommentar „ON CONFLICT … schlägt fehl" war genau dieses Symptom. **Fix DB:** invaliden Index gedroppt + per `CREATE UNIQUE INDEX CONCURRENTLY` sauber neu gebaut (0 Duplikate vorab verifiziert, keine Write-Locks) → Dedup-Query jetzt **Index Only Scan, 3 statt 45.493 Blocks** pro Aufruf (EXPLAIN-verifiziert: 1894 ms → 0,1 ms), `message_id_header`-Eindeutigkeit wieder DB-seitig erzwungen. **Fix Code:** die Mail-Import-Status-Route (`/admin/mail-import/status`) rechnete zwei Full-Table-Aggregate über ~407k Rows bei **jedem** 10s-Dashboard-Poll neu (zweitgrößter IO-Verbraucher: 76 Mio. Blocks) — `totals` + `accounts` jetzt 60s in-memory gecacht; Dashboard-Poll 10s → 30s. **Files:** `backend/src/api/admin/mail-import/status/route.ts`, `backend/src/admin/routes/mail-import/page.tsx`. **Commit:** `c0cbca5`. **Deploy:** Backend-only, VPS Vite-Cache-Clear + PM2-Restart, Backend-Build mit bekannten pre-existing TS-Errors (keine neuen), `/health` 200. **Release-Tag:** [v1.0.0-rc60.2](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc60.2). |
| **v1.0.0-rc60.1** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R2 (Teil 2/2) — `@user`/`@release`-Mentions.** Schließt die R2-Phase des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md) ab. **Composer-Mentions:** ein einzelner `@`-Trigger durchsucht Member **und** Katalog-Releases über die neue Route `GET /store/community/mention-search` (Member nach Handle/Name, Releases über die `Release.search_text`-FTS). Tiptap-Mention-Extension (`CommunityMention`) mit selbst-positioniertem Suggestion-Popup — **keine tippy-Dependency**, Member + Releases teilen eine Ergebnisliste. Mentions serialisieren zu `<span class="cm-mention" data-mention data-mention-type data-id>@label</span>`; die `sanitizeBodyHtml`-Allowlist wurde entsprechend erweitert (`data-mention-type`, `class`, `cm-mention`). **Notifications:** `extractMentions` + `notifyMentions` parsen die Mention-Spans bei Post- und Comment-Erstellung und erzeugen `kind='mention'`-Notifications für jeden erwähnten Member (Selbst-Mentions übersprungen). **Neue Deps:** `@tiptap/extension-mention`, `@tiptap/suggestion`. **Deploy-Vorfall + Recovery:** Der erste Deploy-Versuch lief ins Leere — ein lokal modifiziertes `storefront/package-lock.json` auf dem VPS blockierte `git pull`, und `git pull … | tail -1` maskierte den Fehler vor `set -e` (vgl. memory `feedback_pipefail_ssh_tee`). VPS hing auf `10c8c81`. Recovery: VPS-`package-lock.json` verworfen, sauber gepullt (→ `4a87ecb`), Backend + Storefront neu gebaut. **Verifiziert:** `mention-search?q=tape` → HTTP 200, Member + Releases. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `4a87ecb`. |
| **v1.0.0-rc60.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R2 (Teil 1/2) — Post-Detail, Reactions, Composer.** Erste Hälfte der R2-Phase des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md); Mentions (`@user`/`@release`) folgen als fokussierter nächster Schritt. **Post-Detail (`/community/post/[slug]`):** zweispaltiges Layout — Artikel + Discovery-Sidebar (Linked Release, „More from {Author}", Tags); Editorial-Eyebrow, Lesezeit-Schätzung (`readingTime`), Author-Strip mit Follow-Button. **Reactions per-Emoji:** Backend liefert eine Per-Emoji-Aufschlüsselung auf Posts-Feed + Post-Detail (`fetchReactionBreakdown`-Helper, gruppiert `community_reaction` nach `emoji`); `ReactionsBar` zeigt Counts je Emoji + optimistisches Update; Feed-Cards rendern die Reaction-Zeile (welches Emoji, wie oft) statt einer nackten Gesamtzahl. **Composer:** durchsuchbarer Katalog-Release-Anker-Picker (`ReleasePicker` über `/store/catalog/suggest`, debounced) — ein Release lässt sich jetzt im Composer anheften, nicht mehr nur via `?release_id`-URL-Param; Chip-basiertes Tag-Input (`TagInput`) ersetzt das Komma-getrennte Textfeld. **Neue Komponenten:** `ReleasePicker`, `TagInput`, `ReactionSummary`; CSS-Erweiterung (Post-Detail-Grid, Composer-Felder, responsive). **Deploy:** Backend (Reaction-Breakdown auf 2 Store-Routen) + Storefront, VPS Vite-Cache-Clear + PM2-Restart beider Prozesse, `/health` 200, Reaction-Breakdown in Feed + Detail verifiziert. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Offen für R2 (2/2):** Mentions `@user`/`@release` (Tiptap-Mention-Extension + Mention-Notifications). **Commit:** `10c8c81`. |
| **v1.0.0-rc59.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R1 — Storefront-Shell + Hub auf Mockup-Parität.** Zweite Phase des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md). Die Community-Storefront war eine einspaltige schmale Liste; R1 bringt Shell + Hub auf die Vinyl-Culture-Mockup-Qualität. **Shell:** Sub-Navigation (`Feed · Explore · Lists · Dispatch · Members`), sticky unter dem Site-Header, aktiver Tab aus dem Pfad abgeleitet (`CommunitySubNav`, Client-Komponente mit `usePathname`); persistenter Compose-FAB im `community/layout.tsx`. **Hub neu (`/community`):** Editorial-Hero + zweispaltiges `cm-hub-grid` — Activity-Feed links, Discovery-Sidebar rechts mit 4 Widgets (Active in Auctions, Trending Tags, Suggested Members, From the Catalog). **Neue Backend-Route** `GET /store/community/hub-sidebar` — ein Round-Trip für die gesamte rechte Spalte (Live-Auction-Blocks aus `auction_block`, meistgefolgte Member, kontextuelle Katalog-Picks aus zuletzt geankerten Posts); demo-gegated wie die übrigen Read-Routen. **Aktionsorientierte Empty-States** (Write-a-Post- / Browse-Members-CTA statt „is quiet"). **Responsive `@media`-Layer** ergänzt — die portierte Mockup-CSS stylte Mobile nur über eine JS-`.is-mobile`-Klasse (Device-Frame-Previewer); echte Viewports kollabieren den Hub jetzt auf eine Spalte und machen die Sub-Nav scrollbar. **Coming-Soon-Halteseiten** für Explore (R7), Lists (R5), Members-Directory (R3), damit die Sub-Nav-Links auflösen. **Neue Komponenten:** `CommunitySubNav`, `CommunityWidgets` (4 Sidebar-Widgets), `ComingSoon`; `FollowButton` um `small`-Variante erweitert. **Deploy:** Backend (neue Store-Route) + Storefront, VPS Vite-Cache-Clear + PM2-Restart beider Prozesse, `/health` 200, `hub-sidebar` liefert 6 Member + 3 Katalog-Picks. Typecheck Storefront 0 Errors, Backend ohne neue Errors. **Commit:** `bb50bbe`. |
| **v1.0.0-rc58.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `COMMUNITY_DEMO`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Rebuild R0 — flag-gegateter Demo-Datensatz.** Auftakt des [`COMMUNITY_REBUILD_PLAN.md`](../Community/COMMUNITY_REBUILD_PLAN.md) — der verbindliche 8-Phasen-Plan, der den zu optimistischen „Increment 1–4 = vollständig"-Bauplan ersetzt (die Storefront war eine Skizze, kein mockup-reifes Produkt). **R0 macht die leere Community erlebbar + testbar.** Neuer Feature-Flag `COMMUNITY_DEMO` (default OFF, `requires: COMMUNITY`). Neues idempotentes Seed-Skript `scripts/community_seed.py` (`--load`/`--purge`/`--dry-run`/`--pg-url`): 12 Sammler-Profile inkl. Frank-Curator, 6 Editorials, 20 Discussion-Posts, 88 Comments, 30 Reviews, 190 Reactions, 61 Follow-Kanten — an 16 echte Katalog-Releases geankert. **Demo-Markierung per ID-Präfix** (`cmpro_demo_*`) — keine Schema-Migration, kein Replica-DDL-Risiko, vollständig purge-bar. 6 Community-Read-Routen demo-gegated (`posts`-Feed, `posts/:id`, `comments`, `reviews`, `profiles/:handle`, `tags`): Demo-Inhalt ist unsichtbar solange `COMMUNITY_DEMO` OFF ist. Das Skript korrigiert `Release.averageRating`/`ratingCount` nach dem Seed so, dass Demo-Reviews **nie** echte Katalog-Aggregate verfälschen (verifiziert: 0 Releases mit Demo-Rating-Leak). **Deploy:** Backend (Flag + Gating), VPS Vite-Cache-Clear + PM2-Restart, `/health` 200; Seed auf VPS via `community_seed.py --load`; `COMMUNITY_DEMO` für QA aktiviert. Verifiziert: Feed liefert 26 Posts (6 Editorials), Trending-Tags + Demo-Profile auflösbar. Vor dem Public-Launch wird der Demo-Seed via `--purge` durch den echten FB-Migrations-Content ersetzt. **Commit:** `cd32444`. |
| **v1.0.0-rc57.0** | 2026-05-16 | `beta_test` | `COMMUNITY`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Increments 3 + 4 — sozialer Loop + Kuration/Safety.** Increment 3 + 4 des Community-Plattform-Bauplans, in einem Deploy ausgeliefert. **Increment 3 (sozialer Loop):** 3A Following — `community_follow`-Tabelle, Toggle-Route, Follow-Button, Follower/Following-Profil-Stats. 3B personalisierter Activity-Feed — `?feed=following` (Posts gefolgter Member + Editorials), Hub mit Following/Latest-Tabs (eingeloggte Member landen im personalisierten Feed). 3C Tags-System — Tag-Filter auf der Posts-Route, `/store/community/tags` Trending, `/community/tag/[slug]`-Seiten, klickbare Tags, Trending-Strip im Hub. 3D CRM-Tier-Vererbung — `community_profile.tier` wird per Trigger aus `crm_master_contact.tier` synchronisiert (+ Backfill), Curators ausgenommen. 3E Notifications — `community_notification`-Tabelle, In-App-Benachrichtigungen bei Kommentar/Reply/Follow, `/community/notifications`-Seite. **Increment 4 (Kuration + Safety):** 4A Dispatch — `/community/dispatch` Editorial-Spur. 4B Moderation — `community_report`-Tabelle, Report-Button auf Posts + Comments, Mod-Queue im Community-Admin (Reports-Tab). 4C Trust-Levels — `trust_level`-Spalte (TL0 Newcomer … TL3 Veteran, berechnet aus Alter + Aktivität, Curators TL3), Daily-Post-Rate-Limit pro Level (TL0 5/Tag, TL1 20/Tag). **5 additive Migrationen** (`community_follow`, `community_crm_tier_inheritance`, `community_notification`, `community_report`, `community_trust_level`) — Supabase + Tier-2-Replica gespiegelt, Subscriber refreshed. **Deploy:** Backend + Storefront, `/health` 200, alle Community-Routen 200/401 wie erwartet. Damit ist der Community-Plattform-Bauplan (Increments 1–4) vollständig umgesetzt. |
| **v1.0.0-rc56.0** | 2026-05-15 | `beta_test` | `COMMUNITY`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Community Increment 2 — Medien in Posts + Community-Admin.** Zweiter Increment des Community-Plattform-Bauplans ([`COMMUNITY_PLATFORM_BUILD_PLAN.md`](../Community/COMMUNITY_PLATFORM_BUILD_PLAN.md)). **Medien (Konzept M3):** Bild-Upload nach R2 (`POST /store/community/upload`, WebP-optimiert via sharp) + Tiptap-Image-Button mit File-Picker; Server-Embed-Resolver (`POST /store/community/embed`) für YouTube/Vimeo/Spotify/SoundCloud/Bandcamp mit eigener Tiptap-`Embed`-Node + Editor-Button; Post-Cover-Bild (Upload auf der Compose-Seite, gerendert in Feed-Cards, Editorial-Hero, Post-Detail). HTML-Sanitizer um `div`- + `iframe`-Embed-Allowlist (cm-embed-Klassen-Whitelist) erweitert. **Community-Admin:** neuer 9. Sidebar-Eintrag `/app/community` — Dashboard (Counts + Recent-Activity), Posts-Moderation (Hide/Pin/Remove/Publish), Member-Verwaltung (Tier/Curator/Ban setzen). Neue Admin-Routes `GET /admin/community/dashboard` + `GET /admin/community/profiles`. **Keine DB-Migration** — rein additive Code/Routes. Davor separat: Storefront-Community-UI vollständig auf Englisch korrigiert (rc55.0 war versehentlich Deutsch). **Deploy:** Backend + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200, Community-Routen 200, Embed/Admin-Routes 401 (Auth-Gate korrekt). **Commit:** `aaacc5b`. |
| **v1.0.0-rc55.0** | 2026-05-15 | `beta_test` | `COMMUNITY`, `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **VOD Community Increment 1 — Hub, Posts, Reviews (flag-gated, in dieser Release aktiviert).** Erster Community-Bereich auf vod-auctions.com als Ablösung der Facebook-Gruppe. Gebaut „deploy early, activate when ready" hinter dem neuen Feature-Flag `COMMUNITY` — Phasen 0–5 in einem Zug, Increment-1-Scope bewusst minimal. **DB:** 5 `community_*`-Tabellen (profile/post/comment/reaction/review, additive Migration `community_increment1_schema`) + Rating-Aggregat-Trigger (`community_review_rating_aggregate`) der `community_review`-Bewertungen in die vorhandenen leeren Spalten `Release.averageRating`/`ratingCount` rollt. Schema auf die Tier-2-Replica gespiegelt + `community_*` über `vod_auctions_pub` repliziert (Subscriber refreshed). **Backend:** 11 Routes (`/store/community/*` Posts/Comments/Reactions/Reviews/Profile + `/admin/community/*` Moderation), `lib/community.ts` (Flag-Gate, Profile-Resolution mit Shadow-Profil-Vorbereitung, HTML-Sanitizing via sanitize-html, denormalisierte Counter), Middleware `/store/community/*` mit `allowUnauthenticated`. **Storefront:** `/community` Hub-Feed, Single-Post mit Comments + 7-Emoji-Reactions, Compose mit Tiptap-Editor, Member-Profil (`/community/members/[handle]`) + Profil-Edit (`/community/settings`), Catalog-Anchored Discussion + Reviews (ganzzahlig 1–5 Sterne) auf der Release-Detailseite. Vinyl-Culture-CSS aus dem Mockup portiert. Header + MobileNav: flag-gated Community-Nav-Eintrag. **Flag-Verhalten:** OFF → gesamte `/community`-Surface + Discussion/Review-Tabs antworten 404. Konzept + alle Entscheidungen: `docs/Community/Community Concept.md §17`. **Bewusst NICHT in Increment 1:** Following, Tag-System, Notifications, Moderation-Queue, Trust-Levels, Suche — spätere Increments. **Schritt 2 (offen, separate Überarbeitung):** Facebook-Archiv-Import (5.819 Frank-Posts, Pipeline P1–P5 fertig) + Legacy-Review-Migration aus `3wadmin_tapes_comment` (500 Zeilen). **Commit:** `d29938f` (Feature) + CHANGELOG. **Deploy:** Backend + Storefront, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200, Community-Routen mit Flag ON → 200. |
| **v1.0.0-rc54.5** | 2026-05-15 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Inventory: Erfassung als Landing-Page — Tab-Shell statt „Session"-Schritt.** Robin-Feedback: der Schritt „Session starten" vor dem Erfassen ergab keinen Sinn. **Befund:** die „Stocktake Session" war nie ein echtes Objekt — keine DB-Tabelle, kein Endpoint, kein Lock. „Session starten" = eine Zeile `window.location.href`. Das Vokabular suggerierte eine Zeremonie, die es nicht gab. **Umbau (Konzept [`INVENTORY_ERFASSUNG_LANDING_KONZEPT.md`](../optimizing/INVENTORY_ERFASSUNG_LANDING_KONZEPT.md), Entscheidungen A–D abgezeichnet):** `/app/erp/inventory` ist jetzt eine **Tab-Seite**. Default-Tab **„Erfassung"** = der Arbeitsplatz (Suche + Bewertung + Label-Druck), beim Öffnen sofort nutzbar, Suchfeld fokussiert. Tab **„Übersicht"** = das frühere Dashboard. **Struktur:** `page.tsx` → schlanke Tab-Shell (`PageHeader` + `Tabs` + Lazy-Mount-Hybrid: `ErfassungTab` immer gemountet damit Such-/Edit-State bei Tab-Wechsel erhalten bleibt, `UebersichtTab` erst bei erster Aktivierung — die schweren Aggregat-Queries feuern dann erst, Einstieg ins Inventory wird schneller); `session/page.tsx` → Redirect auf `?tab=erfassung` (alte Bookmarks brechen nicht); `UebersichtTab.tsx` (neu) = Dashboard ohne „Stocktake Session"-Karte; `ErfassungTab.tsx` (neu) = Arbeitsplatz ohne „Exit/Dashboard"-Buttons + Exit-Modal, Keyboard-Shortcuts an `active`-Prop gekoppelt (feuern nicht mehr im inaktiven Tab). **Kompakter Statistik-Block** oben im Erfassungs-Tab (Robin-Wunsch): Verifiziert-Card + Ausstehend-Card + Pro-Person-Tabelle (Frank/David), bewusst kleiner dimensioniert als das Dashboard. `media/[id]` Catalog-Back-Link „← Zurück zur Erfassung" → `?tab=erfassung`. **Reine Frontend-Restrukturierung, keine DB-/Backend-Änderung.** „Session"-Vokabular vollständig entfernt. **Files:** `backend/src/admin/routes/erp/inventory/{page,UebersichtTab,session/page,session/ErfassungTab}.tsx`, `media/[id]/page.tsx`. **Commit:** `f819951` (Commit-Message nennt „rc54.2" — Numbering-Kollision mit parallelem Workstream, Release-Tag ist rc54.5). **Deploy:** Backend-only, VPS Vite-Cache-Clear + PM2-Restart, `/health` 200. |
| **v1.0.0-rc54.4** | 2026-05-15 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Storefront-Catalog-Visibility-Toggle aus `/app/media` entfernt.** Robin-Befund: der „Storefront Catalog Visibility"-Toggle (Filter ON/OFF) auf der Media-Catalog-Seite wird nicht mehr gebraucht — die Steuerung von `site_config.catalog_visibility` läuft über `/app/config` (Access/Launch). Der zweite Editier-Einstieg in `/app/media` war redundant. **Fix:** State (`catalogVisibility`, `catalogVisibilityLoading`), der `/admin/site-config`-Fetch-`useEffect`, die `toggleCatalogVisibility`-Funktion und das UI-Card komplett entfernt — 65 Zeilen. Das Backend-Setting `site_config.catalog_visibility` selbst bleibt unangetastet (steuert weiterhin die Storefront-Sichtbarkeit, toggelbar über `/app/config`). Layout: Suchfeld rückt direkt unter die Stats-Kacheln, ~80–90px vertikal gespart. **Files:** `backend/src/admin/routes/media/page.tsx`. **Commit:** `97e09d0`. **Deploy:** Admin-Route-Änderung → VPS Vite-Cache-Clear (`rm -rf node_modules/.vite .medusa`), Frontend-Build erfolgreich, Backend-Build mit bekannten pre-existing TS-Errors (keine neuen), PM2-Restart, `/health` 200. **Release-Tag:** [v1.0.0-rc54.4](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc54.4). |
| **v1.0.0-rc54.3** | 2026-05-15 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Hotfix rc54.2: Preis-Filter „No"-Fall lieferte falsche Ergebnisse.** Robin's Folge-Analyse (Tapes ohne Legacy-Preis) deckte auf, dass der „No"-Fall der Preis-Filter aus rc54.2 nicht funktionierte. rc54.2 nutzte `shop_price IS NULL` / `legacy_price IS NULL` für „No". **Empirisch verifiziert:** Meilisearch matcht JSON-`null`-Felder **nicht** mit `IS NULL` — global 0 Treffer trotz ~35k null-Docs (Beispiel-Doc `legacy-release-12739` hat nachweislich `legacy_price: null`, wird von `legacy_price IS NULL` aber nicht erfasst); Range-Operatoren matchen `null` ebenfalls nie. Damit lieferten „Shop Price: No" und beide Legacy-Filter-Richtungen falsche Counts (Legacy „Yes" via `IS NOT NULL` matchte sogar alle 52.788). **Fix:** „No"-Fall über das Komplement `NOT <field> > 0`. Da in der DB kein `*_price = 0` existiert (verifiziert: 0 Rows), ist `> 0` exakt deckungsgleich mit „Preis gesetzt". Postgres-Pfade (`route-postgres-fallback.ts` + `media/count/route.ts`) auf identische `> 0`-Semantik gezogen, damit Meili- und Postgres-Backend deckungsgleiche Counts liefern. **Verifikation am Meili-Layer:** `legacy_price > 0` → 16.870, `NOT legacy_price > 0` → 35.917, `shop_price > 0` → 16.855, `NOT shop_price > 0` → 35.927. **Files:** `backend/src/lib/release-search-meili.ts` (buildFilterString), `route-postgres-fallback.ts`, `media/count/route.ts`. Keine Meili-Settings-Änderung nötig. Backend-Build mit bekannten pre-existing TS-Errors, keine neuen. **Commit:** `8c4a99b`. **Release-Tag:** [v1.0.0-rc54.3](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc54.3). |
| **v1.0.0-rc54.2** | 2026-05-15 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Media-Catalog Preis-Filter auf `shop_price` korrigiert + neuer Pictures-Filter.** Robin-Befund: der „Price"-Filter im Admin-Katalog (`/app/media`) hatte keine Wirkung. **Doppelte Ursache:** (a) die aktive Meili-Route `api/admin/media/route.ts` übersetzte den `has_price`-Param nie ins Meili-Filter-Objekt — er wurde still verworfen; (b) der Postgres-Fallback filterte auf `discogs_lowest_price` — die alte Preislogik. Kanonischer Shop-Preis ist laut Preis-Modell rc47.2 `Release.shop_price`; `discogs_lowest_price` ist nur Markt-Referenz. **Fix:** Das einzelne „Price"-Dropdown ist ersetzt durch zwei getrennte Filter — **Shop Price** (`shop_price > 0`, kanonischer Shop-Preis ohne Inventar-Bedingung) und **Legacy Price** (`legacy_price` gesetzt, tape-mag-Historie), je All/Yes/No. Bewusst **kein** Discogs-Preis-Filter (nur Markt-Referenz, nicht filterrelevant) — der bestehende `Discogs`-Filter (= `discogs_id` vorhanden) bleibt unverändert. **Neuer Pictures-Filter** (With/Without pictures, = `coverImage` vorhanden) — `has_image` war backendseitig in Meili-Lib + Route bereits verdrahtet, fehlte nur im UI. **Kachel „WITH PRICE"** zählt jetzt `shop_price > 0` statt `discogs_lowest_price`. **Meili:** `shop_price` + `legacy_price` zu `filterableAttributes` in `meilisearch_settings.json` ergänzt, `--apply-settings` auf beide Indexes angewendet (kein Doc-Re-Push nötig — Felder waren bereits in jedem Doc). **Verifikation am Meili-Layer:** `shop_price > 0` → 16.855 Treffer, `legacy_price IS NOT NULL` → 52.787, `has_image = true` → 44.068. **Files:** `scripts/meilisearch_settings.json`, `backend/src/lib/release-search-meili.ts` (CatalogFilters + buildFilterString), `api/admin/media/route.ts` (Meili-Filter-Mapping), `route-postgres-fallback.ts` + `media/count/route.ts` (Postgres-Filter), `media/stats/route.ts` (Kachel), `admin/routes/media/page.tsx` (UI). Backend-Build mit bekannten pre-existing TS-Errors (memory `feedback_medusa_build_exit_nonzero`), keine neuen. **Commit:** `f042714`. **Release-Tag:** [v1.0.0-rc54.2](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc54.2). |
| **v1.0.0-rc54.1** | 2026-05-15 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Inventory Sale-Mode + Stocktake Condition-Defaults gefixt.** Frank-Befund: neu erfasste Platten landen „immer mal wieder" nicht auf `sale_mode='both'`. **Root-Cause:** die Catalog-Edit-Maske (`/admin/media/:id`) hatte seit 2026-04-22 einen hardcoded Dropdown-Default `direct_purchase`, der bei JEDEM Save unkonditional mitgeschickt wurde (`body.sale_mode = saleMode`) — auch wenn Frank das Sale-Mode-Dropdown nie angefasst, sondern nur z.B. einen Titel korrigiert hat. Releases mit `sale_mode=NULL` wurden so ungefragt auf `direct_purchase` gestempelt. Der Inventory-Verify-Default (`NULL/auction_only → both`) lässt `direct_purchase` danach bewusst als „explizite User-Wahl" unangetastet → erfasste Platte bleibt `direct_purchase` statt `both`. **3 Code-Fixes (rein additiv, keine Datenänderung):** **(A)** `session/page.tsx` — Stocktake-Condition-Defaults Media/Sleeve auf `VG+` als Fallback wenn keine Legacy-Condition parsebar (`startNewCopy` + `startEditCopy`; vorhandene Legacy-Werte unangetastet). **(B)** `media/[id]/page.tsx` — Dropdown-Default `direct_purchase` → `both`, plus `saleModeDirty`-Flag: `sale_mode` wird nur noch persistiert wenn Frank das Dropdown bewusst ändert (`onChange` setzt Flag, `handleSave` sendet `body.sale_mode` nur bei `saleModeDirty`, Reset bei Load + nach erfolgreichem Save). **(C)** `verify/route.ts` — `sale_mode='both'` für Copy #1 aus dem `if(new_price)`-Block herausgezogen, läuft jetzt preis-unabhängig bei jeder Verifizierung; Release-Row einmalig geladen statt 3× separat. **Modell ab jetzt: Default `both` überall, ein Wert.** **Bewusst geparkt (Robin-Decision, Sammel-Bereinigung):** 13.574 Alt-Releases hängen noch auf `direct_purchase` (Bug-Residuum, alle seit 22.04. gestempelt, davon 1.818 bereits inventarisiert). Backfill `UPDATE "Release" SET sale_mode='both' WHERE sale_mode='direct_purchase'` (mit Rollback-Snapshot, Meili-Reindex automatisch da `sale_mode` im `trigger_release_indexed_at_self`-Whitelist) — NICHT jetzt, gesammelt mit anderen Altbestand-Cleanups (TODO.md Now §0). **Files:** `backend/src/admin/routes/erp/inventory/session/page.tsx`, `backend/src/admin/routes/media/[id]/page.tsx`, `backend/src/api/admin/erp/inventory/items/[id]/verify/route.ts`. **Commit:** `b468a56`. **Deploy:** Backend-only (Admin-/API-Routen), VPS PM2-Restart, `/health` 200. |
| **v1.0.0-rc54.0** | 2026-05-11 | `beta_test` | `ERP_INVENTORY`, `SEARCH_MEILI_CATALOG`, `SEARCH_MEILI_ADMIN`, `SYSTEM_HEALTH_*` | **Country-ISO Migration — Single-Session Cutover (Variante A-Express).** Robin's Catalog-Test deckte auf, dass `Release.country` mit 3 verschiedenen Encodings befüllt war (English Names 88.6 % · UK/US-Aliase 4.6 % · Bereits-ISO 4.2 % · Multi-Region 1.3 % · NULL 8.3 %). Read-Pfade widersprachen sich: Admin-UI warf `⚠️ (non-ISO)`, Storefront-Filter mappte auf English, Meili erwartete ISO. **Migration auf ISO-3166-1 alpha-2 als Single Source of Truth in einer ~6h-Session** (statt 72h Plan v1 mit 48h-Beobachtungsphase). Variante A-Express mit dual-tolerant Read-Pfaden (`whereIn(country, [iso, name])`) ab Phase 1 — Backfill ist zero-impact für Storefront-Filter. **Numbers:** 48.694 Rows backfilled (DE 12.706 · US 11.159 · GB 10.994 · 59 distinct ISO-Codes), 0 leftover_dirty, 4.094 NULL, CHECK-Constraint `release_country_iso_format` aktiv. **Multi-Region-Strategie:** Pure-Europe → `EU` (ISO-3166-1 exceptionally-reserved, 🇪🇺-Flag), Worldwide → `WO` (VOD-intern, ISO-W-Range unassigned seit Dekaden), Compound „UK & Europe"/„USA & Canada" → primary country (Sekundärinfo verloren bei 154 Rows = 0.29 %), Region-Sammelnamen Benelux→NL/Scandinavia→SE → traditionelles Press-Land, deprecated ISO-3166-3 (YU/DD/CS/SU) erhalten für historische Releases. **Phase 0 Sicherheitsnetze ersetzen 48h-Beobachtung:** Snapshot-Backup (48.743 Rows), Discogs-Cache-Audit (71 distinct Strings in 90d-Window → 2 Lücken entdeckt + ergänzt: „UK & France", „Australia & New Zealand"), Live-API-Probe (10 Roundtrips), Sandbox-Branch Test-Cutover mit 99 synthetic Rows ($0.02 — entdeckte den Bedarf für case-insensitive Backfill-Matching). **Code-Architektur:** shared `lib/country-normalize.ts::normalizeCountryToIso()` für alle 5 Write-Pfade (legacy_sync, discogs-import/commit, discogs-preview, manual-edit, picker), `lib/country-resolve.ts::resolveCountryForFilter()` dual-tolerant für Storefront-PG-Fallback, `admin/data/country-iso.ts` +6 Entries (EU/WO/YU/DD/CS/SU) + 35 neue Aliases + `formatCountryCompact()` für 90px Stocktake-Spalte. Python-Pendant `scripts/data/country_iso.py` mit defensiver `lookup_iso()`-Identity-Passthrough (kritisch: verhindert dass alte Meili-Sync-Version nach Backfill `country_code: null` schreibt). **255 Meili-Synonyms** generiert via `scripts/build_country_synonyms.py` — bi-direktional „germany"↔„deutschland"↔„DE", „england"↔„uk"↔„GB", „europa"↔„europe"↔„EU" etc. Volltextsuche findet weiterhin Country-Strings obwohl Docs nur ISO enthalten. **Display final:** Admin Catalog-Detail rendert `formatCountryLabel(release.country) = "🇫🇷 France (FR)"` wie Frank-Wunsch (Screenshot 2026-05-11), Stocktake-Column kompakt `formatCountryCompact(r.country) = "🇩🇪 DE"`, Storefront-Badge raw ISO. **Tests:** 138 TS unit-tests (country-normalize + country-resolve) + 20 Python (country_iso_test), alle grün. **Frank-Workflow „germany"-Volltextsuche:** 13.947 hits (höher als 12.706 DE-Rows allein wegen Synonym-Expansion in Multi-Region-Compounds + Typo-Tolerance), „england" → 11.144 hits. **Critical Review aufgedeckt:** 2 kritische Bugs in Plan v1 (Meili-`lookup_iso(ISO)` returnt None nach Backfill → country_code-null in 46k Docs; Storefront `COUNTRY_ALIASES` mappt auf English-Name nach Backfill → 0 Treffer für `?country=Germany`) — beide via Variante A's dual-tolerant Strategie + defensive `lookup_iso` umgangen. **Out of Scope für rc54.0 (Follow-up RSE):** ~~PressOrga.country (1.983 Rows in deutschen Vollformen — selbe Methode, ~2h), LabelPerson.country (458 Rows)~~ — **am selben Tag (2026-05-11) als RSE-324 nachgezogen.** PressOrga 1.457 ISO + 526 NULL · LabelPerson 447 ISO + 11 NULL · 0 leftover_dirty · CHECK-Constraints `pressorga_country_iso_format` + `labelperson_country_iso_format` aktiv. Bonus: identische CHECK-Constraints auch auf `Artist`, `Label`, `musician` angelegt (alle 0 Non-NULL-Rows, dirty-Write-Schutz für Zukunft). **Migration:** [`2026-05-11_pressorga_labelperson_country_iso_backfill.sql`](../../backend/scripts/migrations/2026-05-11_pressorga_labelperson_country_iso_backfill.sql). **Commit:** `fd0ce20`. **Linear:** [RSE-324](https://linear.app/rseckler/issue/RSE-324) closed. Storefront-Filter-Cleanup (dual-tolerant → single-value, kein Performance-Issue) bleibt Backlog. **Snapshot-Drop** automatisch nach 7 Tagen (2026-05-18). **PR:** [#1](https://github.com/rseckler/VOD_Auctions/pull/1). **Release-Tag:** [v1.0.0-rc54.0](https://github.com/rseckler/VOD_Auctions/releases/tag/v1.0.0-rc54.0). **Doku:** [Plan](../optimizing/COUNTRY_ISO_MIGRATION_PLAN.md), [Implementation](../optimizing/COUNTRY_ISO_MIGRATION_IMPLEMENTATION.md), [Critical Review](../optimizing/COUNTRY_ISO_MIGRATION_CRITICAL_REVIEW.md), [Backfill-SQL](../../backend/scripts/migrations/2026-05-11_country_iso_backfill.sql). **5 Commits:** `46fa014` Foundation, `b365f4f` Write-Pfade, `ab9761d` Backfill-SQL, `3ba955d` Read-Cleanup, `71f2b09` Merge PR #1. |
