...
**Last Updated:** 2026-04-24 (rc49.2 → rc49.7 — 6-RC-Ops-Day. Alert-Storm-Aufarbeitung + Pricing-Cleanup. (rc49.2) Meili-Sync chunked fetch + Backlog-Alert. (rc49.3) Legacy-Sync `statement_timeout=5min` + `cleanup_stale_runs()` + Probe-Timeout 5s→10s. (rc49.4) **Root-Cause-Fix:** Legacy-Sync `ON CONFLICT DO UPDATE SET … WHERE <semantic-diff>` — hourly 41k-Meili-Cascade strukturell beendet (180s → 47s, Meili-Traffic −99.97%). (rc49.5) Sentry-API-Query ohne `OR` — HTTP 400 im System-Health-Drawer weg. (rc49.6) Storefront Pricing-Cleanup: `effective_price` end-to-end, `legacy_price` raus aus saved/wins/auction-detail, `enrichWithShopPrice()` auf saved + recommendations. (rc49.7) **Bug-Fix:** `legacy_available` aus `is_purchasable`-Gate raus — 36 Releases mit `legacy_available=false` trotz verifiziertem Bestand waren fälschlich versteckt, jetzt wieder kaufbar. Meili Full-Rebuild 52.783 × 2.)
...
1. **Storefront Pricing Cleanup komplett (rc49.6 + rc49.7, 2026-04-24).** `effective_price = shop_price` end-to-end, `legacy_price` raus aus Storefront. `/store/account/saved` + `/store/account/recommendations` via `enrichWithShopPrice()` (shop_price > 0 AND EXISTS(verified erp_inventory_item)). Auction-Detail-Seite: "Catalog Price"-Row entfernt. **rc49.7 kritischer Fix:** `legacy_available` aus `is_purchasable`-Gate raus — 36 Releases mit verifiziertem Bestand aber `legacy_available=false` (tape-mag historisch verkauft) waren fälschlich nicht kaufbar. Franks `price_locked=true` ist jetzt alleinige Shop-Visibility-Authority. Meili Full-Rebuild 52.783 × 2. Doku: [`docs/architecture/PRICING_MODEL.md`](docs/architecture/PRICING_MODEL.md) + [`docs/optimizing/FRONTEND_PRICING_CLEANUP_PLAN.md`](docs/optimizing/FRONTEND_PRICING_CLEANUP_PLAN.md).
2. **Legacy-Sync WHERE-gated UPSERT (rc49.4, 2026-04-24).** **Root-Cause-Fix für stündliche 41k-Meili-Cascade.** Trigger `release_indexed_at_self` feuerte bei jedem UPDATE wegen MySQL→PG-Type-Coercion-Mismatches. Fix: `ON CONFLICT DO UPDATE SET ... WHERE <semantic-diff>`. Live-Messung: rows_written=41547, nur 13 echte UPDATEs, Legacy-Sync 180s → 47s (4×), Meili-Traffic −99.97%. Plus rc49.2/rc49.3 Meili-Sync chunked fetch + statement_timeout=5min + stale-cleanup + Backlog-Alert + Probe-Timeout 10s.
3. **Meili-Backlog-24h-Uptime-Stripe noch rot (64 %)** — lagging indicator, rollt bis morgen 06:30 UTC aus dem Window. Seit rc49.4-Deploy (06:30 UTC) keine neuen Red-Samples.
4. **Alle Admin+Storefront-Listen auf Meilisearch (rc49.1, 2026-04-23 Abend).** Vier Endpoints live: `/store/catalog` (seit rc40), `/admin/media` (rc48.1), `/admin/erp/inventory/browse` + `/admin/erp/inventory/search` (rc49.1). Scanner-Pattern VOD-XXXXXX/VOD-\d+ bleiben Postgres. **Tag 3 Frontend-Polish** (Skeleton + React-Query + Optimistic-Updates + Prefetch) folgt separat.
5. **Preis-Modell komplett (rc47.2 + rc47.3).** `Release.shop_price` einziger kanonischer Shop-Preis. Verify/Add-Copy schreiben `shop_price` + Defaults `sale_mode='both'` + `warehouse_location_id=ALPENSTRASSE`. Auction-Start-Preis = `round(shop_price × default_start_price_percent / 100)`.
6. **System Health Observability komplett live (rc41-rc47.1).** 25 Checks, Sampler-Architektur, 7-stufiges Severity-Modell, Alert-History + Auto-Resolve, Sentry-Embed (rc49.5-Fix für Boolean-Query), Log-Drawer, Low-Impact-Actions, Public Status Page, 7 Runbooks. P4-E destructive Actions OFFEN — nach 4 Wochen re-evaluieren.
7. **Frank arbeitet aktiv an Inventur.** MacBook-Air-Rollout ausstehend: `cd ~/VOD_Auctions && git pull && bash frank-macbook-setup/install.sh`.
8. **POS P0 Dry-Run live** — Frank testet Scan→Cart→Checkout, Feedback sammeln.
9. **L1:** AGB-Anwalt beauftragen (Launch-Blocker, RSE-78).
10. **`supabase_realtime: degraded`** (known, non-blocker): Realtime-Service aktivieren sobald Live-Bidding live geht.
