...
1. **Discogs-ID-Change → Preview-Modal mit Per-Field-Review (rc51.9.2, 2026-04-26).** „Save Linking" mit geänderter `discogs_id` ruft jetzt `POST /admin/media/:id/discogs-preview` auf, das `releases`/`marketplace/stats`/`price_suggestions` von discogs.com holt und einen Diff `{current, proposed, has_changes}` ohne DB-Write zurückgibt. `DiscogsReviewModal` zeigt Per-Field-Diff mit Checkboxen — locked-Felder default OFF mit 🔒-Badge, „Select all/none". Apply schreibt durch denselben `POST /admin/media/:id`-Pfad → Audit-Log + Auto-Lock + Meili-Push laufen normal. „Refetch from Discogs" routet ebenfalls durch das Preview — kein silent overwrite mehr. Discogs-Country (Name) wird via neuem `findCountryByName()` (`country-iso.ts`) auf ISO-2 normalisiert. Out-of-scope: Artist/Label/Cover-Image (dedizierte Picker, würden DB-Inserts triggern). Plus rc51.9.1: kritischer JSONB-Array-Binding-Fix in `POST /admin/media/:id` — node-postgres serialisierte `format_descriptors`-JS-Array als PG-`text[]`, PG hat keinen `text[] → jsonb`-Cast → Save lieferte „An unknown error occurred." Fix: `JSON.stringify` vor `trx.update()` damit der `text → jsonb`-Assignment-Cast greift.
...
9. **Frank briefen auf rc50.0 → rc51.9.2 UI-Neuerungen:** Edit-Stammdaten für ALLE Releases (auch Legacy), per-Field 🔒-Icons + Unlock-Modal, Country-Picker mit Flag + EN/DE-Search, Barcode strict-Validation, neuer Discogs-ID-Change-Review-Modal. MacBook-Air-Rollout ausstehend.
