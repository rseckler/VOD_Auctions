...
1. **Preis-Modell komplett (rc47.2 + rc47.3, 2026-04-23).** `Release.shop_price` ist **einziger** kanonischer Shop-Preis. `legacy_price` + `discogs_lowest_price` nur noch Info. Verify/Add-Copy schreiben `shop_price` + setzen Defaults `sale_mode='both'` + `warehouse_location_id=ALPENSTRASSE`. Storefront-Gate via `catalog_visibility`. **Phase 2 (rc47.3):** Auction-Start-Preis = `round(shop_price × default_start_price_percent / 100)` mit Fallback shop_price → estimated_value → legacy_price → 400. Neuer Bulk-Rule `shop_price_percentage` für Block-weites Re-Pricing. Backfill rc47.2: 23 verifizierte Items, 22 × `sale_mode=both`, 32 × Warehouse. Doku: [`docs/architecture/PRICING_MODEL.md`](docs/architecture/PRICING_MODEL.md).
