...
**Aktive Workstreams:**
1. **Frank's Bulk-Invite-Test-Welle starten (rc53.17 ✅ live, 2026-05-10)** — Custom `/store/customer/register`-Endpoint und der refactored `/store/invite/[token]`-Pfad sind beide hinter dem `invite_mode_active`-Gate und unter Production-Concurrency validiert. **Nächste Aktion:** Frank wählt im `/app/crm` 10-20 Test-Kontakte aus der Smart-List "📨 Newsletter Subscribers" und löst die Bulk-Invite-Action aus. Open Items aus dem Workstream §14 (Job-Monitor-Page B.5, Re-Opt-In-Mode für `/newsletter`, Anwalts-Check §7(3)-Wording mit RSE-78) bleiben Backlog. Doku: [Session-Log](docs/sessions/2026-05-09_rc53_17_register_endpoint.md), [B2C-Funnel-Verification](docs/optimizing/B2C_REGISTRATION_FUNNEL_VERIFICATION.md), [Plan + Codex-Review](docs/optimizing/CUSTOM_REGISTER_ENDPOINT_PLAN.md).
2. **Artist-Duplikate-Cleanup (rc53.16, NEU 2026-05-09)** — Bowie als erster Case live (`David Bowie` einheitlich auf 99 Releases). Audit-Doc [`docs/audit_artist_duplicates_2026-05-09.md`](docs/audit_artist_duplicates_2026-05-09.md) zeigt **2.063 weitere exakt-Name-Duplikate / 4.131 Artist-Rows / 16.585 Releases (~40 % Katalog)** plus ~50 Klasse-B-„X" vs „The X"-Cases. **Nächste Aktion:** Frank-Briefing zum Audit-Doc; gemeinsam Top-30 nach Impact (Various, Depeche Mode, Kraftwerk, Joy Division, Pink Floyd, John Coltrane, The Cure, Bauhaus, Sex Pistols, Killing Joke, Tangerine Dream, Miles Davis etc.) als Tier-1-Batch durchgehen. Sync-Pipeline-Härtung als RSE-Ticket (Slug-Check vor INSERT bei legacy_sync_v2 + discogs-import/commit).
3. **CRM Master v1 + Email Review ✅ live (rc53.1)** — vollständig nutzbar auf `/app/crm` inkl. Tab "Email Review" für die 976 Stage-4 Candidates. **Nächste Aktion:** Frank-Einarbeitung (10-Tab-Drawer + Bulk-Actions + Smart-Lists + Email-Review-Workflow), Pre-2019-MO-PDFs aus Robin's Backup-Archiv ins `Monkey Office/Rechnungen/<Jahr>/`-Folder kopieren. Doku: [`docs/architecture/CRM_SYSTEM_STATE_2026-05-04.md`](docs/architecture/CRM_SYSTEM_STATE_2026-05-04.md) + [Session-Log](docs/sessions/2026-05-04_crm_master_v1_buildout.md).
4. **AGB-Anwalt beauftragen** (RSE-78, **High**) — Launch-Blocker für die ersten öffentlichen Auktionen.
5. **Frank-MBA-Rollout** für rc52.x Catalog-Edit-UI (Edit-Stammdaten für ALLE Releases, per-Field 🔒-Icons + Unlock-Modal, Country-Picker, Barcode-GTIN, Discogs-Review-Modal, Storefront-Updates ~1s nach Edit). Briefing ausstehend.
6. **POS P0 Dry-Run** — Frank testet Scan→Cart→Checkout mit `TSE='DRY_RUN'`. P1 wartet auf Steuerberater-Freigabe.
7. **MiniMax Phasen 3-5** geplant (Foundation rc52.7 done): Hailuo Auction-Block-Teaser-Videos, `image-01` Cover-Placeholder für ~3.500 Releases ohne Cover, M2-Bulk-Backfill für 22.630 NULL-Genre/Style-Cases. Phasen 3-4 brauchen OpenClaw-Top-up; DSGVO-Check für AI-Chat-Tools (Haiku) vor Switch zu M2 nötig. Doku: [`PROJECTS/docs/MINIMAX_INTEGRATION.md`](../docs/MINIMAX_INTEGRATION.md).
8. **RSE-321 Tape-mag-Multi-Artist** (2026-05-03 angelegt, Backlog) — `legacy_sync_v2.py:624` packt Komma-haltige `band_name`-Strings als ein Artist; ~133 Komma-Cases + 18 Featuring + 12 Vs. zur manuellen Klassifikation. Folgt RSE-320 Option A (Discogs-Pfad rc52.12 done). **Nebenbefund aus rc53.16-Audit:** zerstückelte Multi-Artist-`band_name`-Strings haben Einzel-Artist-Rows produziert (`legacy-artist-11105` „FROM", `-6505` „Tape", `-8268` „NO", `-5820` „Sound" etc. — vermutlich „FROM TAPE NO SOUND"-Splitting).
9. **`supabase_realtime: degraded`** (non-blocker) — Realtime-Service aktivieren sobald Live-Bidding live geht.
