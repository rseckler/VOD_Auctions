...
**Last Updated:** 2026-04-24 (rc50.0 → rc50.4 — **Catalog Stammdaten-Editierbarkeit Phase 1-4 deployed.** Frank kann ab jetzt Stammdaten von Discogs-Import-Releases (~11k) editieren während Legacy (~41k) read-only bleiben. (rc50.0, Opus 4.7) Backend-Foundation: 4-Zonen-Modell, Migration `data_source` + `release_audit_log`-Tabelle, 8 neue Routes (`/audit-log`, `/audit-log/:id/revert`, `/tracks`, `/tracks/:id`), Helper `release-source.ts` + `release-audit.ts` mit `revertEntry()` (FOR UPDATE Locks, looseEqual für DECIMAL-Roundtrip, RevertError-Class mit 5 Codes). 2 Codex-Rescue-Bugs gefixt (camelCase `updatedAt` + Lost-Update-Race). (rc50.1+rc50.1.1, Sonnet+Opus-Review) Frontend Edit-Mode: SourceBadge, LockBanner, PickerModals (Artist/Label-Picker mit trgm-Suggest), shared `validateReleaseStammdaten()`-Library FE+BE. 4 Bugs nach Opus-Review behoben (CSS-var-concat, POST-Response-JOINs, Validation-Library-Wiring, dead Doc-Link). (rc50.2+rc50.3, Sonnet+Opus) AuditHistory-Tab + TrackManagement + RevertConfirmModal mit 4 Views (confirm / 409-conflict-diff / locked / gone), `unwrapAuditValue()` für double-encoded JSON. (rc50.4, Sonnet+Opus-Review) Bulk-Edit Skip-Logic für Hard-Stammdaten (legacy releases werden übersprungen, Toast "X updated · Y skipped"), Final Architecture-Review GREEN über alle 5 Release-Writer.)
...
1. **Catalog Stammdaten-Editierbarkeit komplett live (rc50.0 → rc50.4, 2026-04-24).** 4-Zonen-Modell: Zone 0 (System-IDs, immer locked), Zone 1 (Hard-Stammdaten title/year/country/artistId/labelId/catalogNumber/barcode/description/coverImage — editable nur bei `data_source=discogs_import`, locked bei Legacy), Zone 2 (genres/styles/credits/barcode — always open), Zone 3 (Commerce — unverändert). **Backend:** Migration `data_source` Backfill (legacy:41553, discogs_import:11230) + `release_audit_log`-Tabelle + 8 Routes (GET/POST `/admin/media/:id/tracks`, PATCH/DELETE `/.../tracks/:id`, GET `/admin/media/:id/audit-log`, POST `/audit-log/:id/revert` mit force-Override). `revertEntry()` mit FOR UPDATE Locks + `looseEqual()` (DECIMAL-Roundtrip) + 5 RevertError-Codes. **Frontend:** Edit-Card im Detail (SourceBadge, LockBanner, Artist/Label-Picker trgm-suggest), AuditHistory-Tab, RevertConfirmModal mit 4 Views (confirm/409-conflict-diff/locked/gone), TrackManagement-Card, Bulk-Edit "Stammdaten (skips legacy)" Optgroup mit Summary-Toast "X updated · Y skipped". **Geprüft:** 4 Phase-2-Bugs + 2 Codex-Rescue-Bugs (updatedAt-camelCase + Lost-Update-Race) gefunden+gefixt. Final Opus-Architecture-Review GREEN. Shared Validation `validateReleaseStammdaten()` FE+BE. `pushReleaseNow()` in allen 9 Writer-Paths für sofortige Meili-Reindex. Doku: [`docs/optimizing/CATALOG_STAMMDATEN_EDITABILITY_KONZEPT.md`](docs/optimizing/CATALOG_STAMMDATEN_EDITABILITY_KONZEPT.md) + [`docs/optimizing/IMPLEMENTATION_PLAN.md`](docs/optimizing/IMPLEMENTATION_PLAN.md).
2. **⚠ Pre-existing Bug entdeckt beim rc50.4-Deploy:** `payment-deadline` Cron crasht täglich 09:00 UTC mit `column "updated_at" of relation "Release" does not exist`. Gleicher Bug-Typ wie Codex-Rescue-Fix in Phase 1 — camelCase `updatedAt` vs snake_case. Existiert seit mindestens rc49.x, nicht aus Phase 1-4. Next Action: grep `"Release".*update.*updated_at` im Codebase + rc50.5-Patch.
...
8. **Frank arbeitet aktiv an Inventur + nutzt jetzt Catalog-Stammdaten-Edit.** MacBook-Air-Rollout ausstehend.
