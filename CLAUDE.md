...
**Last Updated:** 2026-05-17 — **rc72.1 Compilation-Tracklist-Rebuild + Codex-Review-Fixes.** ~2.200 „Various"-Compilations hatten Legacy-`Track.title` als zusammengeklatschten Müll-String — komplett aus Discogs neu aufgebaut (`rebuild_compilation_tracklists.py`): 2.203 Releases / 38.266 Tracks, 0 Fehler. Plus 3 Codex-Review-Findings gefixt (Track-Trigger `TG_OP`-Branch + `search_indexed_at`-Bump, `discogs-import/fetch` cached jetzt per-track `artists`). Davor **rc71.5/rc71.6 Discogs-Tracklist: Per-Track-Künstler.** rc71.5 fixte den Compilation-Künstler-Verlust beim Fetch (regression aus rc69) — backte ihn aber als `"Artist – Titel"`-String in `Track.title`. rc71.6 macht ihn zu einer **strukturierten Spalte `Track.artist_name`** → suchbar (Meili `track_artists` + `search_text`-Trigger) **und** klickbar (Storefront-Tracklist verlinkt auf die Künstler-Seite, read-time Slug-Auflösung). Un-Bake: 6.071 Tracks / 420 Compilations. Konzept: [`TRACK_ARTIST_STRUKTURIERT_KONZEPT.md`](docs/optimizing/TRACK_ARTIST_STRUKTURIERT_KONZEPT.md). Davor **Discogs-Toolchain rc69.0–rc71.3.** **rc69.0 Inventory Discogs Fetch:** Der Katalog-Discogs-Flow (Verlinken/Refetch) schreibt jetzt auch Marktpreise + Tracklist (vorher nur Genres/Styles) — der Inventory-Process zeigt den „Markt aktuell"-Block damit sofort statt erst nach dem Cron; der Erfassungs-Tab (`/app/erp/inventory`) hat eine immer sichtbare „DISCOGS"-Sektion zum Verknüpfen/Fetchen direkt im Inventory-Prozess (geteilter `DiscogsReviewModal`). **rc70.0 Discogs-Backfill-Tool** (`/app/discogs-backfill`, Sidebar-Shortcut): Gap-Analyse der 5.211 verifizierten Releases ergab 1.246 mit fehlenden Genres/Styles/Credits/Tracklist (der Cron schreibt nur Preise) — das Tool zeigt sie in einer Review-Tabelle `current → proposed`, Frank akzeptiert pro Zeile/Bulk, **nichts automatisch**; Apply ist rein additiv. **rc71.1 Hotfix:** Tier-2-Replikation stand ~7 min — die rc70-Tabelle fehlte auf der Replica (`vod_auctions_pub` ist schema-weit → jede neue `public`-Tabelle wird auto-publiziert und MUSS vor dem ersten Write auch auf der Replica angelegt werden; Lehre in DB-Gotcha „Logical Replication" verankert). **rc71.3 Codex-Review-Fixes:** 3 Findings — Tracklist-Wipe (P1, leere Discogs-Tracklist hätte vorhandene Tracks löschen können), `job_running`-Hänger nach Backend-Restart (P2), Markt-only-Apply blockiert (P2). Konzepte: [`INVENTORY_DISCOGS_FETCH_KONZEPT.md`](docs/optimizing/INVENTORY_DISCOGS_FETCH_KONZEPT.md) · [`DISCOGS_BACKFILL_TOOL_KONZEPT.md`](docs/optimizing/DISCOGS_BACKFILL_TOOL_KONZEPT.md) · [`VERIFIED_RELEASES_DISCOGS_GAP_ANALYSIS_2026-05-16.md`](docs/optimizing/VERIFIED_RELEASES_DISCOGS_GAP_ANALYSIS_2026-05-16.md). Davor **VOD Community Rebuild R0–R9 + Erweiterung 01+02** (rc58–rc71.2, Flag `COMMUNITY` ON) — As-Built [`docs/Community/COMMUNITY_SYSTEM_STATE.md`](docs/Community/COMMUNITY_SYSTEM_STATE.md). Volle Release-Historie: [`docs/architecture/CHANGELOG.md`](docs/architecture/CHANGELOG.md). Operative Arbeitsliste: [`docs/TODO.md`](docs/TODO.md).
...
- **⚠️ Logical Replication — neue Tabelle MUSS auf die Replica gespiegelt werden:** `vod_auctions_pub` ist eine **schema-weite** Publication (`FOR TABLES IN SCHEMA public`). Jede neue Tabelle im `public`-Schema wird dadurch **automatisch publiziert** — `ALTER PUBLICATION … ADD TABLE` ist unnötig (und ein No-Op). Konsequenz: **jede neue `public`-Tabelle MUSS dieselbe DDL auch auf der `pg17-replica` (DB `vod_auctions_replica`) bekommen, BEVOR die erste Zeile geschrieben wird** — sonst crash-loopt der Apply-Worker (`target relation "public.…" does not exist`) und die GESAMTE `vod_auctions_sub`-Replikation steht (rc70.0-Vorfall: `discogs_backfill_candidate` vergessen → Replikation 7 min tot). Es gibt **kein „nicht replizieren"** für eine `public`-Tabelle. Ablauf: (1) `CREATE TABLE` auf Supabase, (2) identische `CREATE TABLE` auf der Replica via `docker exec -u postgres pg17-replica psql -d vod_auctions_replica`, (3) `ALTER SUBSCRIPTION vod_auctions_sub REFRESH PUBLICATION WITH (copy_data=false)`. Spalten-Adds: Replica zuerst. CHECK-Constraint-Änderungen ebenfalls auf die Replica.
...
- 11 `community_*`-Tabellen, alle auf `vod_auctions_replica` repliziert. Neue DDL immer spiegeln — **siehe DB-Gotcha „Logical Replication"** (schema-weite Publication, jede neue `public`-Tabelle MUSS vor dem ersten Write auch auf der Replica angelegt werden, sonst steht die Replikation)
...
- `Artist` (12.451), `Label` (3.077), `PressOrga` (1.983), `Format` (39), `Image` (+`rang`), `Track` (+`artist_name` rc71.6: Per-Track-Künstler bei Compilations), `ReleaseArtist`
...
- Discogs-Backfill (rc70): `discogs_backfill_candidate` (release_id UNIQUE, discogs_id, status `fetch_pending`→`pending`→`applied`/`rejected`/`error`, gaps `text[]`, proposed `jsonb`) — Staging-Tabelle für das Backfill-Review-Tool `/app/discogs-backfill`. Transient, aus Discogs rekonstruierbar
...
- Discogs Backfill (rc70): `/discogs-backfill` (GET list+counts+job-state), `/discogs-backfill/{prepare,apply,reject}` — Review-Tool für fehlende Genres/Styles/Credits/Tracklist auf verifizierten Releases. Admin-UI `/app/discogs-backfill` (Sidebar-Shortcut). Apply ist rein additiv. Plus auf `/media/:id`: `discogs-preview` (rc69: + Marktpreise + Tracklist im Diff), `refetch-discogs`
...
├── lib/                     # stripe/paypal/shipping/brevo/crm-sync/site-config/invite/feature-flags/paths/release-search/release-search-meili/meilisearch/image-upload/email/shop-price/discogs-backfill.ts
