...
- **Discogs Import Architektur (v5.2):** Sessions in `import_session`, Events in `import_event`, Cache in `discogs_api_cache`. Shared Lib: `backend/src/lib/discogs-import.ts` (`SSEStream`, `emitDbEvent`, `getSession`, `isCancelRequested`, `pushLastError`, `awaitPauseClearOrCancel(pg, id, stream | null)`). Shared React-Components: `backend/src/admin/components/discogs-import.tsx`. **Fetch (rc18): Detached Background Task** — POST spawnt `runFetchLoop()` via `void … .catch()`, returnt sofort 200 JSON, UI polled (2s) `/session/:id/status?since_id=X`. Idempotent via 60s Stale-Check. Fetch-Progress wird alle 10 Iterationen geschrieben. **Analyze/Commit** nutzen noch SSE-Streams (Cancel während Commit → `throw "__CANCEL__"` → Transaction-Rollback). Siehe `docs/DISCOGS_IMPORT_SERVICE.md` v5.2.
- **HTTP-Request-lifetime vs Background-Loop:** Lang laufende Loops (Fetch, Analyze, Commit) dürfen nicht tightly mit `res.write()` gecoupled sein. Wenn Client disconnected (Navigation, Tab-Close, pm2 restart), tear-down des Medusa-Request-Scopes killed den Loop **still** (keine Exception im Stderr). Lösung: Loop als detached task via `void runLoop(...).catch(...)`, HTTP-Route returnt 200 sofort, Events werden via `emitDbEvent(pg, sessionId, phase, type, payload)` in `import_event` geschrieben, UI polled. Pattern siehe `backend/src/api/admin/discogs-import/fetch/route.ts` (rc18). Analyze/Commit sind noch nicht decoupled — Follow-up.
